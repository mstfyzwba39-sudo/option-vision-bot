...
def _strong_company_attribution(symbol, headline, summary=""):
    ...
    # Short/common tickers are too risky as bare words. Require explicit market context.
    risky_short_tickers = {"CAT", "ARM", "BA", "MU", "OPEN", "GS", "CRM"}
    ticker_pattern = rf"(?<![a-z0-9]){re.escape(symbol.lower())}(?![a-z0-9])"
    ticker_in_headline = bool(re.search(ticker_pattern, headline_norm))

    if symbol in risky_short_tickers:
        market_context_markers = [
            "stock", "shares", "nasdaq", "nyse", "ticker", "earnings",
            "revenue", "guidance", "investor", "sec filing", "price target",
            "analyst", "company", "ceo", "cfo", "board", "quarter",
        ]
        if ticker_in_headline and any(x in combined for x in market_context_markers):
            return True
        return False
