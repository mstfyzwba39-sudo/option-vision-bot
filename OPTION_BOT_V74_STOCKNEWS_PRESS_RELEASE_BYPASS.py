# OPTION_BOT V70 - News source diagnostics for Stock News API / Finnhub / Google RSS
import os
import time
import math
import asyncio
...
def get_stocknews_primary_news(force=False):
    """Premium primary feed with safe staged fallback for 403/parameter issues."""
    if not STOCK_NEWS_API_TOKEN:
        STOCK_NEWS_DIAG.update({"http": None, "received": 0, "normalized": 0, "cache_hit": False, "error": "TOKEN_MISSING"})
        print("V74 STOCKNEWS TOKEN_MISSING", flush=True)
        return []

    now = time.time()
    cached = STOCK_NEWS_CACHE.get("items")
    if (not force and cached is not None and
            now - float(STOCK_NEWS_CACHE.get("time") or 0) < STOCK_NEWS_API_CACHE_SECONDS):
        STOCK_NEWS_DIAG.update({"received": len(cached), "normalized": len(cached), "cache_hit": True, "error": None})
        print(f"V74 STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)

    # Stage 1: official minimal request shape. This isolates auth/account issues
    # from filters, ticker batching and optional fields.
    probe_params = {"tickers": "AMD", "items": 3, "page": 1, "token": STOCK_NEWS_API_TOKEN}
    try:
        probe, probe_body = _stocknews_request(probe_params)
        print(f"V74 STOCKNEWS PROBE HTTP={probe.status_code} | BODY={probe_body or '[empty]'}", flush=True)
        if probe.status_code != 200:
            err = f"PROBE_HTTP_{probe.status_code}: {probe_body or probe.reason}"
            STOCK_NEWS_CACHE["error"] = err
            STOCK_NEWS_DIAG.update({"http": probe.status_code, "received": 0, "normalized": 0, "cache_hit": False, "error": err})
            return list(cached or [])

        # Stage 2: broad multi-ticker request, kept intentionally simple.
        main_params = {
            "tickers": ",".join(STOCK_NEWS_COMPANY_SYMBOLS),
            "items": STOCK_NEWS_API_ITEMS,
            "page": 1,
            "token": STOCK_NEWS_API_TOKEN,
        }
        main, main_body = _stocknews_request(main_params)

        # Some accounts/gateways can reject very large ticker lists. If so,
        # transparently retry in small chunks using the same documented endpoint.
        raw_items = []
        if main.status_code == 200:
            ...
            if not isinstance(raw_items, list):
                raw_items = []
            print(f"V74 STOCKNEWS MAIN HTTP=200 | RECEIVED={len(raw_items)}", flush=True)
        else:
            print(f"V74 STOCKNEWS MAIN HTTP={main.status_code} | BODY={main_body or '[empty]'} | RETRY=CHUNKS", flush=True)
            chunk_size = 5
            seen_ids = set()
            for i in range(0, len(STOCK_NEWS_COMPANY_SYMBOLS), chunk_size):
                chunk = STOCK_NEWS_COMPANY_SYMBOLS[i:i+chunk_size]
                cp = {"tickers": ",".join(chunk), "items": 3, "page": 1, "token": STOCK_NEWS_API_TOKEN}
                cr, cb = _stocknews_request(cp)
                print(f"V74 STOCKNEWS CHUNK {','.join(chunk)} HTTP={cr.status_code}", flush=True)
                if cr.status_code != 200:
                    continue
                pdata = cr.json()
                part = pdata.get("data") if isinstance(pdata, dict) else pdata
                if not isinstance(part, list):
                    continue
                for raw in part:
                    key = str(raw.get("news_id") or raw.get("id") or raw.get("news_url") or raw.get("title")) if isinstance(raw, dict) else repr(raw)
                    if key not in seen_ids:
                        seen_ids.add(key)
                        raw_items.append(raw)

        STOCK_NEWS_DIAG.update({"http": 200 if raw_items is not None else main.status_code, "received": len(raw_items), "cache_hit": False, "error": None})
        items = []
        for raw in raw_items:
            item = _stocknews_normalize_item(raw)
            if item and item.get("datetime"):
                items.append(item)
        STOCK_NEWS_CACHE.update({"time": now, "items": items, "error": None})
        STOCK_NEWS_DIAG.update({"normalized": len(items), "error": None})
        print(f"V74 STOCKNEWS SUCCESS | RECEIVED={len(raw_items)} | NORMALIZED={len(items)} | SYMBOLS={sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        for idx, it in enumerate(items, 1):
            print(
                f"V74 STOCKNEWS ITEM {idx}/{len(items)} | TICKERS={it.get('stocknews_tickers')} | "
                f"SOURCE={it.get('source') or '?'} | RANK={it.get('stocknews_rank_score')} | "
                f"SENTIMENT={it.get('stocknews_sentiment')} | HEADLINE={str(it.get('headline') or '')[:220]}",
                flush=True,
            )
        return list(items)
    except Exception as exc:
        ...
        print(f"V74 STOCKNEWS ERROR: {safe} — using fallback feeds", flush=True)
        return list(cached or [])
...
def scan_manual_stock_news():
    ...
    try:
        for raw in get_stocknews_primary_news():
            ...
            if not syms:
                print(
                    f"V74 STOCKNEWS REJECT | SYMBOL=? | STAGE=SYMBOL_RESOLUTION | REASON=NO_TRACKED_SYMBOL | "
            ...
            for sym in syms:
                ...
                if not scored:
                    print(
                        f"V74 STOCKNEWS REJECT | SYMBOL={sym} | STAGE=MANUAL_SCORE | "
    ...
    out.sort(key=lambda x:(-x.get("importance",0), -_news_source_rank(x), -x.get("timestamp",0)))
    capped=[]; counts={}
    for x in out:
...
def _v67_finalize_stock_news(items):
    ...
    for raw in (items or []):
        ...
        if not x:
            if raw.get("stocknews_primary") or str(raw.get("news_provider") or "").upper() == "STOCKNEWS":
                print(
                    f"V74 STOCKNEWS REJECT | SYMBOL={raw.get('symbol') or '?'} | STAGE=FINAL_GATE | "
    ...
    cleaned.sort(key=lambda x:(-int(x.get("importance") or 0), -_v68_provider_priority(x)[0], -_v68_provider_priority(x)[1], -_news_source_rank(x), -int(x.get("timestamp") or 0)))
...
def format_combined_news():
    ...
    stock_items.sort(key=lambda x: (-x.get("importance", 0), -_v68_provider_priority(x)[0], -_v68_provider_priority(x)[1], -_news_source_rank(x), -x.get("timestamp", 0)))
    ...
    print(
        f"V73 DIAG STOCKNEWS HTTP={STOCK_NEWS_DIAG.get('http')} | "
...
async def post_init(application):
    ...
    if STOCK_NEWS_API_TOKEN or FINNHUB_TOKEN:
        ...
        if STOCK_NEWS_API_TOKEN:
            ...
            print("V70 NEWS SOURCE DIAGNOSTIC ENABLED — provider tags only in Render logs")
        else:
