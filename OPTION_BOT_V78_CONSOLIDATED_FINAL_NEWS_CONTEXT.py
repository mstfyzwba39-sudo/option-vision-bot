# OPTION_BOT V78 - Consolidated final + contextual news materiality
import os
import time
import math
import asyncio
    ...
    while True:
        ...
        for key, watch in list(
            ...
            try:
                ...
                active_key = f"{chat_id}:{watch['option_symbol']}"
                        ...
                        if quote and float(quote.get("ask", 0) or 0) > 0:
                            ...
                            if not item.get("reverse_sent"):
                                ...
                                if risk_level == "EXIT":
                                    ...
                                    if hist.get("status") not in {"SUCCESS", "LOSS"}:
                                        hist["status"] = "STOPPED"
                                    hist["latest_price"] = price
                ...
                for chat_id in ALLOWED_USERS:
                    ...
                    def fmt_group(title, group):
                        ...
                        for _, h in group:
                            ...
                            lines.append(
                                f"• {_active_contract_line(h)}\n"
                                f"  دخول ${entry:.2f} | أعلى ${high:.2f} ({best:+.1f}٪) | آخر {latest:+.1f}٪"
                            )
                        return "\n".join(lines)
