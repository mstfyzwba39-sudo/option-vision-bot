...
def get_company_news(symbol, days=NEWS_DAYS):
    if not FINNHUB_TOKEN or not str(FINNHUB_TOKEN).strip():
    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 429:
                last_error = RuntimeError("FINNHUB_429_RATE_LIMIT")
                if attempt < 2:
            ...
            if response.status_code in {401, 403}:
                raise RuntimeError(
                    f"FINNHUB_AUTH_{response.status_code}"
                )
            ...
            if not isinstance(data, list):
                raise ValueError(
                    "استجابة الأخبار غير متوقعة."
                )

            return data

        except requests.Timeout as exc:
            last_error = RuntimeError("FINNHUB_NEWS_TIMEOUT")
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
...
def scan_top10():
    ...
    for symbol in SCAN_SYMBOLS:
        try:
            ...
            # Added display metrics only; qualification logic below stays unchanged.
            metrics = opportunity_metrics(
                symbol,
                trend=trend,
                contracts=result["contracts"],
            )
            entry_zone = get_entry_zone_status(symbol, trend["bias"])
