...
def format_top10(results):
    ...
    lines = [
        ...
        (
            "🕒 السوق مفتوح — تأكيد 15د لكل فرصة"
            if market_open
            else "🕒 السوق مغلق — تأكيد 15د بعد الافتتاح"
        ),
        "━━━━━━━━━━━━",
        "",
    ]
    ...
    for idx, item in enumerate(results, 1):
        contract = item["best_contract"]
        zone = item.get("entry_zone", {})
        zone_status = zone.get("status", "UNKNOWN")
        zone_text = zone.get("zone_text", "—")

        if zone_status == "INSIDE":
            entry_text = f"🟢 داخل {zone_text}"
        elif zone_status == "NEAR":
            entry_text = f"🟡 قريب {zone_text}"
        elif zone_status == "WAIT":
            entry_text = f"⏳ ينتظر {zone_text}"
        elif zone_status == "INVALID":
            entry_text = f"🔴 خارج {zone_text}"
        else:
            entry_text = "⚪ المنطقة غير متاحة"

        lines.extend([
            (
                f"{idx}️⃣ {item['symbol']} "
                f"{contract['side']} "
                f"{contract['strike']:g} | "
                f"{format_expiration_date(contract['expiration'])}"
            ),
            (
