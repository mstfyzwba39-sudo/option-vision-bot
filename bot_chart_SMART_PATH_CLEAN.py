...
def make_chart(symbol, resolution):
    ...
    # Smart conditional path — always starts after the last candle.
    path_x0 = x_last + 1.15
    trigger = setup["trigger"]
    confirm_buffer = max(avg_range * 0.20, last_price * 0.001)
    bullish = trigger >= last_price
    confirmed = (last_price >= trigger + confirm_buffer) if bullish else (last_price <= trigger - confirm_buffer)
    path_color = gold if bullish else invalid

    if not confirmed:
        # Before confirmation: do NOT pretend all targets are already expected.
        # Show a modest attempt, a small retest, then the confirmation level only.
        move = trigger - last_price
        p1 = last_price + move * 0.48
        p2 = last_price + move * 0.28
        # Wider, smoother visual route: advance -> shallow retest -> confirmation.
        # Keep every point strictly to the right of the final candle.
        p1 = last_price + move * 0.34
        p2 = last_price + move * 0.27
        p3 = last_price + move * 0.64
        path_x = [path_x0, path_x0 + 2.0, path_x0 + 3.6, path_x0 + 5.8, path_x0 + 8.0]
        path_y = [last_price, p1, p2, p3, trigger]
        label = "WAIT → CONFIRM"
    else:
        # After confirmation: retest the breakout/breakdown, then progress to targets.
        retest = trigger + (avg_range * 0.22 if bullish else -avg_range * 0.22)
        path_x = [path_x0, path_x0 + 1.5, path_x0 + 2.8, path_x0 + 4.1,
                  path_x0 + 5.4, path_x0 + 6.7]
        path_y = [last_price, retest, targets[0], targets[1], targets[2], targets[3]]
        label = "CONFIRMED PATH"

    ax.plot(path_x, path_y, color=path_color, linewidth=2.15,
            linestyle=(0, (5, 3)), alpha=0.95, zorder=2)
    label_y = path_y[-2] if not confirmed else path_y[1]
    label_offset = max(avg_range * 0.24, last_price * 0.0010)
    ax.text((path_x[-2] - 0.8) if not confirmed else (path_x0 + 1.65),
            label_y + (label_offset if bullish else -label_offset),
            label, color=charcoal, fontsize=10.8, fontweight="bold",
            va="bottom" if bullish else "top",
            bbox={"boxstyle":"round,pad=0.14","facecolor":"#FFFDF8",
                  "edgecolor":"#D7C9A4","linewidth":0.6}, zorder=6)
