...
def make_chart(symbol, resolution):
    ...
    if demand_zone:
        ...
        ax.axhspan(
            ...
            facecolor=gold_soft, alpha=0.34, zorder=0,
        )
        ax.text(
            demand_start + 0.2, demand_upper + max(last_price * 0.004, 0.5),
            "DEMAND ZONE",
            color="#1F1F1F", fontsize=16.5, fontweight="bold", va="bottom", zorder=2,
        )
    ...
    if chart_targets:
        ...
        if direction == "BULLISH":
            ...
            # Neutral: do not draw an unrealistic jump from current price to a distant trigger.
            # The projected path starts at confirmation and is visually conditional.
            confirmed = False
            points_y = [trigger]
        ...
        if len(points_y) >= 2:
            span = min(10.0, future_bars - 1.0)
            step = span / max(1, len(points_y) - 1)
            points_x = [x_last + 0.55 + i * step for i in range(len(points_y))]
            ax.plot(
                points_x, points_y, color=path_color, linewidth=2.8, alpha=0.96,
                linestyle="--" if direction == "NEUTRAL" else "-",
                solid_capstyle="round", solid_joinstyle="round", zorder=5,
            )
            ax.scatter(points_x, points_y, s=22, color=path_color, zorder=6)
            ax.annotate(
                "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
                arrowprops={"arrowstyle": "-|>", "color": path_color, "lw": 3.0, "alpha": 0.98},
                zorder=6,
            )
            label = "EXPECTED PATH" if direction != "NEUTRAL" else "PATH AFTER CONFIRM"
            label_i = min(1, len(points_y) - 1)
            ax.text(
                points_x[label_i] + 0.15, points_y[label_i] + atr_local * 0.75, label, color="#202020", fontsize=13.5,
                fontweight="bold", va="bottom", zorder=6,
                bbox={"boxstyle":"round,pad=0.20","facecolor":"#FFFDF8","edgecolor":gold_soft,"linewidth":0.8,"alpha":0.94},
            )
    ...
    ax.set_xlim(-0.8, future_end + 4.0)
