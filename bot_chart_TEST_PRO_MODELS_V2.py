...
def _detect_clean_setup(highs, lows, closes):
    ...
    for lookback in (20, 24, 28, 32, 36, 42, 48):
        ...
        h = highs[-lookback:]
        l = lows[-lookback:]
        ph = _pivot_points(h, 'high', 2)[-5:]
        pl = _pivot_points(l, 'low', 2)[-5:]
    ...
    for a in range(max(0, len(ph)-6), len(ph)-2):
        ...
        if h2[1]-max(h1[1],h3[1]) < atr*0.65: continue
        neck=(n1[1]+n2[1])/2
        height=h2[1]-neck
    ...
    for a in range(max(0, len(pl)-6), len(pl)-2):
        ...
        if min(l1[1],l3[1])-l2[1] < atr*0.65: continue
        neck=(n1[1]+n2[1])/2
        height=neck-l2[1]
        if height < atr*1.6: continue
    ...
    # DOUBLE TOP / DOUBLE BOTTOM
    # -----------------------------------------------------
    for i in range(max(0,len(ph)-5),len(ph)-1):
        p1,p2=ph[i],ph[i+1]
        sep=p2[0]-p1[0]
        if not 5 <= sep <= 26: continue
        tops_tol=max(atr*0.75,last_price*0.014)
        if abs(p1[1]-p2[1]) > tops_tol: continue
        mids=_between(pl,p1[0],p2[0])
        if not mids: continue
        trough=min(mids,key=lambda p:p[1])
        height=((p1[1]+p2[1])/2)-trough[1]
        if height < atr*1.35: continue
        trigger=trough[1]-atr*0.08; invalid=max(p1[1],p2[1])+atr*0.18
        targets=[trigger-height*x for x in (0.30,0.50,0.72,1.0)]
        score=73 + min(9,height/atr)
        candidates.append({'score':score,'type':'double_top','name_ar':'قمة مزدوجة','direction':'BEARISH',
            'lookback':lookback,'trigger':trigger,'invalidation':invalid,'targets':targets,
            'pattern_lines':[[(float(x),float(y)) for x,y in (p1,trough,p2)]],
            'label_xy':((p1[0]+p2[0])/2,max(p1[1],p2[1])+atr*0.3),'confidence':int(min(88,score))})

    for i in range(max(0,len(pl)-5),len(pl)-1):
        p1,p2=pl[i],pl[i+1]
        sep=p2[0]-p1[0]
        if not 5 <= sep <= 26: continue
        bottoms_tol=max(atr*0.75,last_price*0.014)
        if abs(p1[1]-p2[1]) > bottoms_tol: continue
        mids=_between(ph,p1[0],p2[0])
        if not mids: continue
        peak=max(mids,key=lambda p:p[1])
        height=peak[1]-((p1[1]+p2[1])/2)
        if height < atr*1.35: continue
        trigger=peak[1]+atr*0.08; invalid=min(p1[1],p2[1])-atr*0.18
        targets=[trigger+height*x for x in (0.30,0.50,0.72,1.0)]
        score=73+min(9,height/atr)
        candidates.append({'score':score,'type':'double_bottom','name_ar':'قاع مزدوج','direction':'BULLISH',
            'lookback':lookback,'trigger':trigger,'invalidation':invalid,'targets':targets,
            'pattern_lines':[[(float(x),float(y)) for x,y in (p1,peak,p2)]],
            'label_xy':((p1[0]+p2[0])/2,min(p1[1],p2[1])-atr*0.3),'confidence':int(min(88,score))})
    ...
    if candidates:
        ...
        for c in candidates:
            ...
            max_age = 10 if not str(c.get('type', '')).startswith('triangle') else 14
            ...
            if abs(last_price - trigger) > atr * 4.5:
                continue
...
def make_chart(symbol, resolution):
    ...
    if pattern_lines:
        ...
        for line in pattern_lines:
            ...
            ax.plot(xs, ys, color=gold, linewidth=3.0, alpha=0.95, zorder=5)
            ax.scatter(xs, ys, s=20, color=gold, zorder=6)
    ...
    if demand_zone:
        ...
        ax.text(
            ...
            color="#9A741B", fontsize=13.5, fontweight="bold", va="bottom", zorder=2,
        )
    ...
    # Expected path: state-aware scenario. It does not jump back to a distant confirmation
    # once confirmation has already happened, and it uses a nearby demand/retest when logical.
    if chart_targets:
        ...
        if direction == "BULLISH":
            ...
            if not confirmed:
                if demand_zone:
                    ...
                    # Only show a demand retest if it is reasonably close; never draw a huge plunge.
                    if 0 < last_price - demand_mid <= atr_local * 3.0:
                        points_y.append(demand_mid)
                points_y.append(trigger)
            else:
                # After breakout, a shallow retest is more realistic than returning to an old level.
                retest = max(trigger, last_price - atr_local * 0.65)
                if retest < last_price - atr_local * 0.18:
                    points_y.append(retest)

            future_targets = [t for t in chart_targets if t > last_price + atr_local * 0.08]
            if not future_targets:
                future_targets = chart_targets[-2:]

        elif direction == "BEARISH":
            confirmed = last_price <= trigger
            if not confirmed:
                # Before breakdown: a modest rejection/retest, then confirmation.
                retest = min(setup["invalidation"] - atr_local * 0.25, last_price + atr_local * 0.70)
                if retest > last_price + atr_local * 0.18:
                    points_y.append(retest)
                points_y.append(trigger)
            else:
                # After breakdown: retest the break only when it is close enough to remain plausible.
                if 0 < trigger - last_price <= atr_local * 2.2:
                    retest = min(trigger, last_price + atr_local * 0.70)
                    if retest > last_price + atr_local * 0.18:
                        points_y.append(retest)
            ...
            # Neutral: show only the conditional path after confirmation; no directional invention.
            confirmed = False
            points_y.append(trigger)
            future_targets = chart_targets

        # Add up to four future milestones with one small pullback between T1 and T2.
        future_targets = future_targets[:4]
        if future_targets:
            points_y.append(future_targets[0])
            if len(future_targets) > 1:
                pullback = (future_targets[0] + (trigger if not confirmed else last_price)) / 2
                points_y.append(pullback)
                points_y.extend(future_targets[1:])

        # De-duplicate nearly identical points so the path reads clearly.
        clean_points = []
        for y in points_y:
        ...
        if len(points_y) >= 2:
            span = min(17.5, future_bars - 1.5)
            step = span / max(1, len(points_y) - 1)
            points_x = [x_last + 0.55 + i * step for i in range(len(points_y))]
            ax.plot(
                points_x, points_y, color=path_color, linewidth=4.6, alpha=0.94,
                solid_capstyle="round", solid_joinstyle="round", zorder=5,
            )
            ax.scatter(points_x, points_y, s=40, color=path_color, zorder=6)
            ax.annotate(
                "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
                arrowprops={"arrowstyle": "-|>", "color": path_color, "lw": 4.4, "alpha": 0.98},
                zorder=6,
            )
            label = "EXPECTED PATH" if direction != "NEUTRAL" else "PATH AFTER CONFIRM"
            label_i = 1 if len(points_y) > 2 else 0
            ax.text(
                points_x[label_i], points_y[label_i], label, color=path_color, fontsize=14.5,
    ...
    if setup.get("type") == "structure":
        ax.text(
            0.012, 0.965, "NO CLEAR PATTERN", transform=ax.transAxes,
            fontsize=12.8, fontweight="bold", color="#7A7467", va="top",
            bbox={"boxstyle":"round,pad=0.28","facecolor":"#F8F5EC","edgecolor":"#D8D0B9","linewidth":0.9},
            zorder=8,
        )
    ...
    ax.tick_params(axis="y", colors="#77736C", labelsize=11.5, length=0, pad=4)
