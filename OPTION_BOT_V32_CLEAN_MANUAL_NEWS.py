...
def format_news_item(item):
    ...
    headline = translate_headline_ar(original_headline)

    # V32: manual news UI must stay Arabic. If online translation fails and
    # returns an English title, show a safe Arabic summary instead.
    arabic_chars = len(re.findall(r"[\u0600-\u06FF]", headline))
    latin_chars = len(re.findall(r"[A-Za-z]", headline))
    if latin_chars > max(12, arabic_chars * 2):
        company_name = {
            "TSLA": "تسلا", "MSFT": "مايكروسوفت", "NVDA": "إنفيديا",
            "AAPL": "آبل", "SPCX": "سبيس إكس", "BA": "بوينغ",
            "CAT": "كاتربيلر", "AMD": "AMD", "META": "ميتا",
            "AMZN": "أمازون", "GOOGL": "ألفابت", "ORCL": "أوراكل",
            "CRM": "سيلزفورس",
        }.get(str(item.get("symbol") or "").upper(), str(item.get("symbol") or "السهم"))
        headline = (
            f"خبر {str(item.get('category_ar') or 'مهم')} متعلق بـ{company_name} "
            f"— راجع المصدر للتفاصيل"
        )
