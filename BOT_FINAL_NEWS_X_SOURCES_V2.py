...
# Optional fast X/Twitter monitoring through public Nitter-compatible RSS.
# No X password/token is stored in the bot. If the RSS proxy is unavailable,
# the normal Finnhub news path keeps working without interruption.
X_RSS_PROXY = os.environ.get("X_RSS_PROXY", "https://twiiit.com").rstrip("/")
TRUSTED_X_ACCOUNTS = {
    "DeItaone": "Walter Bloomberg",
    "unusual_whales": "Unusual Whales",
    "StockMKTNewz": "Stock Market News",
}
X_NEWS_TIMEOUT_SECONDS = 12
X_AUTO_MIN_IMPORTANCE = 8
...
NEWS_AUTO_INTERVAL_SECONDS = 300
NEWS_AUTO_MIN_IMPORTANCE = 8
NEWS_AUTO_MAX_AGE_HOURS = 18  # لا نفوّت خبراً مهماً إذا تأخر المصدر/تعطل الفحص
MAX_NEWS_RESULTS = 10
...
def get_trusted_x_news():
    """Fetch recent posts from a small curated set of fast market-news X accounts.

    Uses a Nitter-compatible RSS proxy, so it does not require the user's X login.
    Public RSS proxies can occasionally be unavailable/rate-limited; failures are
    isolated and never stop Finnhub or the rest of the bot.
    """
    items = []
    headers = {
        "User-Agent": "Mozilla/5.0 (OptionBot News Monitor/1.0)",
        "Accept": "application/rss+xml, application/atom+xml, text/xml, */*",
    }

    for username, display_name in TRUSTED_X_ACCOUNTS.items():
        url = f"{X_RSS_PROXY}/{username}/rss"
        try:
            response = requests.get(
                url,
                headers=headers,
            ...
            for entry in entries[:20]:
                ...
                # Nitter titles sometimes start with "R to @...:" for replies or
                # contain repost prefixes. We keep only original/quoted market posts
                # that can be attributed to the curated account itself.
                title_norm = _normalize_news_text(title)
                if title_norm.startswith("r to @"):
                    continue

                items.append({
                    "id": f"x:{username}:{link or timestamp}:{title}",
                    "headline": title,
                    "summary": description,
                    "datetime": timestamp,
            ...
            print(f"X NEWS ERROR @{username}: {type(exc).__name__}: {exc}", flush=True)
...
def _symbols_mentioned_in_news(headline, summary=""):
    ...
    for symbol in SCAN_SYMBOLS:
        if str(symbol).upper() in {"SPY", "QQQ", "IWM"}:
            continue
        if _headline_mentions_symbol(symbol, headline):
            found.append(symbol)
            continue
        # X posts sometimes put the company only in a second line/description.
        # For curated sources we allow the full post body to resolve the symbol,
        # but score_news_item still requires the material-news trigger itself.
...
def score_news_item(symbol, item):
    ...
    if not headline_subject_match:
        if item.get("trusted_x") and symbol in _symbols_mentioned_in_news(headline, summary):
    ...
    recall_scale_patterns = [
        r"\brecall(?:s|ed|ing)?\b[^\n]{0,80}\b(?:[2-9](?:\.\d+)?\s*million|[2-9],?\d{2},?\d{3})\b",
        r"\b(?:[2-9](?:\.\d+)?\s*million|[2-9],?\d{2},?\d{3})\b[^\n]{0,80}\bvehicle(?:s)?\b[^\n]{0,60}\brecall",
    ]
...
def scan_important_news():
    ...
    # Fast X layer: only three curated market-news accounts. The same strict
    # company relevance + material-event scoring is reused, so ordinary posts,
    # opinions, previews, memes and low-impact chatter are ignored.
    try:
        for item in get_trusted_x_news():
...
async def monitor_important_news(
    ...
    while True:
        try:
            ...
            # لا نبتلع الأخبار الموجودة عند إعادة تشغيل Render.
            # SENT_NEWS_IDS محفوظة على القرص وتمنع التكرار، لذلك نفحص الأخبار
            # المهمة الموجودة ضمن نافذة العمر المسموحة ونرسل غير المرسل منها.
            if not baseline_ready:
                baseline_ready = True
                print("NEWS MONITOR READY — CHECKING UNSENT RECENT STORIES")

            for item in results:
                min_importance = (
                    X_AUTO_MIN_IMPORTANCE
                ...
                if age_seconds < -300 or age_seconds > (NEWS_AUTO_MAX_AGE_HOURS * 3600):
                    SENT_NEWS_IDS.add(news_id)
                    _save_auto_sent_state()
                    continue

                alert_header = (
                    "⚡ خبر عاجل من مصدر سوق موثوق على X"
                    if item.get("trusted_x")
                    else "🚨 خبر مهم جديد"
                )
                text = (
