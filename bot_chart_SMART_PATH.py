...
def make_chart(symbol, resolution):
    ...
    if not confirmed:
        ...
        path_x = [path_x0, path_x0 + 1.6, path_x0 + 2.9, path_x0 + 4.4]
        path_y = [last_price, p1, p2, trigger]
        label = "WAIT → CONFIRM"
    else:
    ...
    ax.plot(path_x, path_y, color=path_color, linewidth=1.9,
            linestyle=(0, (4, 3)), alpha=0.95, zorder=2)
    label_y = path_y[1]
    label_offset = max(avg_range * 0.32, last_price * 0.0012)
    ax.text(path_x0 + 1.65, label_y + (label_offset if bullish else -label_offset),
