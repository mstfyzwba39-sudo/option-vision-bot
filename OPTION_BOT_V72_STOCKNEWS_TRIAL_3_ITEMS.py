...
def get_stocknews_primary_news(force=False):
    ...
    if not STOCK_NEWS_API_TOKEN:
        ...
        print("V72 STOCKNEWS TOKEN_MISSING", flush=True)
        return []
    ...
    if (not force and cached is not None and
        ...
        print(f"V72 STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)
    ...
    try:
        ...
        print(f"V72 STOCKNEWS PROBE HTTP={probe.status_code} | BODY={probe_body or '[empty]'}", flush=True)
        ...
        if main.status_code == 200:
            ...
            if not isinstance(raw_items, list):
                raw_items = []
            print(f"V72 STOCKNEWS MAIN HTTP=200 | RECEIVED={len(raw_items)}", flush=True)
        else:
            print(f"V72 STOCKNEWS MAIN HTTP={main.status_code} | BODY={main_body or '[empty]'} | RETRY=CHUNKS", flush=True)
            chunk_size = 5
            seen_ids = set()
            for i in range(0, len(STOCK_NEWS_COMPANY_SYMBOLS), chunk_size):
                ...
                print(f"V72 STOCKNEWS CHUNK {','.join(chunk)} HTTP={cr.status_code}", flush=True)
        ...
        print(f"V72 STOCKNEWS SUCCESS | RECEIVED={len(raw_items)} | NORMALIZED={len(items)} | SYMBOLS={sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        return list(items)
    except Exception as exc:
        ...
        print(f"V72 STOCKNEWS ERROR: {safe} — using fallback feeds", flush=True)
        return list(cached or [])
...
def format_combined_news():
    ...
    print(
        f"V72 DIAG STOCKNEWS HTTP={STOCK_NEWS_DIAG.get('http')} | "
