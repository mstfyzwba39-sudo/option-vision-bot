...
# V65: broader MANUAL stock-news view. Auto alerts keep the strict material scanner unchanged.
def _manual_news_type(item):
    text = _normalize_news_text(f"{item.get('headline','')} {item.get('summary','')}")
    if any(x in text for x in ["ceo", "chief executive", "cfo", "chairman", "president", "jensen huang", "lisa su", "elon musk", "says", "said", "speaks", "comments", "remarks"]):
        return "🎙️ تصريحات الإدارة"
    if any(x in text for x in ["upgrade", "downgrade", "price target", "analyst", "rating", "initiates coverage"]):
        return "🧮 محللون وتقييمات"
    if any(x in text for x in ["shares rise", "shares fall", "stock rises", "stock falls", "stock jumps", "stock drops", "premarket", "pre-market", "sector", "stocks in focus"]):
        return "📊 حركة السهم/القطاع"
    if any(x in text for x in ["analysis", "what to watch", "why ", "outlook", "investors", "report"]):
        return "📰 تغطية وتحليل"
    return "🏢 خبر شركة مباشر"

def _score_manual_context_news(symbol, item):
    # First keep every story that passes the existing strict material rules.
    strict = score_news_item(symbol, item)
    if strict:
        strict["news_type_ar"] = "🏢 خبر شركة مباشر"
        return strict
    headline = str(item.get("headline") or "").strip()
    summary = str(item.get("summary") or "").strip()
    ts = int(item.get("datetime") or 0)
    if not headline or not ts or time.time() - ts > NEWS_MAX_AGE_HOURS * 3600 or time.time() - ts < -300:
        return None
    if str(symbol).upper() in {"SPY","QQQ","IWM"}:
        return None
    if not _strong_company_attribution(symbol, headline, summary):
        return None
    if not _headline_mentions_symbol(symbol, headline) and not (item.get("trusted_fast_news") and symbol in _symbols_mentioned_in_news(headline, summary)):
        return None
    # Manual context may include reputable coverage/statements; keep weak aggregators out.
    probe={"source": item.get("source") or ""}
    if _news_source_rank(probe) < 82 and not item.get("trusted_fast_news"):
        return None
    typ = _manual_news_type(item)
...
def scan_manual_stock_news():
    # Start with strict stories, then add reputable same-day context without changing auto alerts.
    out=list(scan_important_news() or []); seen={(x.get("url") or x.get("headline") or "").lower().strip() for x in out}
    pools=[]
    for fn in (get_fast_backup_news, get_batched_material_news, get_priority_event_backup_news):
        try: pools.extend(fn() or [])
        except Exception as exc: print(f"V65 MANUAL CONTEXT {fn.__name__}: {type(exc).__name__}: {exc}", flush=True)
    for raw in pools:
    ...
    out.sort(key=lambda x:(-x.get("importance",0), -x.get("timestamp",0)))
    return out
...
def format_combined_news():
    ...
    try:
        ...
        if not stock_items:
            print("V64 MANUAL STOCK NEWS: strict Finnhub + Google RSS scan returned no material company stories", flush=True)
    except Exception as exc:
    ...
    b=("🏢 أهم أخبار أسهمنا\n━━━━━━━━━━━━━━\n\n"+"\n\n━━━━━━━━━━━━━━\n\n".join(formatted_stock_items)
       if formatted_stock_items else f"🏢 أخبار أسهمنا\n\n✅ لا توجد أخبار شركة مباشرة أو تصريحات أو تغطيات موثوقة مرتبطة بأسهمنا خلال آخر {NEWS_MAX_AGE_HOURS} ساعة.")
    return a+"\n\n━━━━━━━━━━━━━━\n\n"+b
