...
FAST_NEWS_AUTO_MIN_IMPORTANCE = 9
...
def _material_query_for_symbol(symbol):
    ...
    material = (
        ...
        '"CEO resigns" OR "CEO steps down" OR "FDA approval" OR "FDA rejects"'
    )
...
def score_news_item(symbol, item):
    ...
    # Directional-only news mode.
    # Headline signals are intentionally weighted more than summary/context so a clearly
    # negative event (recall/strike/rejection/scaled-back plan) cannot be diluted into NEUTRAL.
    headline_pos = sum(1 for x in POSITIVE_WORDS if x in headline_norm)
    headline_neg = sum(1 for x in NEGATIVE_WORDS if x in headline_norm)
    body_pos = sum(1 for x in POSITIVE_WORDS if x in combined)
    body_neg = sum(1 for x in NEGATIVE_WORDS if x in combined)
    pos = headline_pos * 3 + body_pos
    neg = headline_neg * 3 + body_neg

    # Strong event-level overrides for common high-impact headlines.
    negative_event_patterns = [
        r"\bunion\b.{0,80}\breject(?:s|ed)?\b.{0,80}\bcontract\b",
        r"\breject(?:s|ed)?\b.{0,80}\bcontract\b",
        r"\bauthoriz(?:e|es|ed|ing)\b.{0,40}\bstrike\b",
        r"\bstrike\b.{0,40}\bauthoriz",
        r"\brecall(?:s|ed|ing)?\b",
        r"\bscale(?:s|d)? back\b",
        r"\bcut(?:s)? plans\b",
        r"\border(?:s|ed)?\b.{0,80}\bclos(?:e|ed|ure)\b",
        r"\bsafety (?:issue|defect|risk)\b",
    ]
    positive_event_patterns = [
        r"\bgain(?:s|ed)?\b.{0,80}\bafter\b.{0,80}\bapproval\b",
        r"\brise(?:s|n)?\b.{0,80}\bafter\b.{0,80}\bapproval\b",
        r"\bapproval\b.{0,80}\b(?:permit|license|robotaxi)\b",
        r"\bexpand(?:s|ed)?\b.{0,80}\b(?:ties|partnership|relationship)\b",
        r"\bwin(?:s)?\b.{0,60}\b(?:contract|order)\b",
        r"\bsecure(?:s|d)?\b.{0,60}\b(?:contract|order)\b",
    ]
    if any(re.search(p, headline_norm, re.I) for p in negative_event_patterns):
        neg += 10
    if any(re.search(p, headline_norm, re.I) for p in positive_event_patterns):
        pos += 10
    ...
    if pos >= 1 and pos > neg:
        ...
        # User preference: do not show or push neutral/unclear company news at all.
        print(f"NEWS DROP UNCLEAR/NEUTRAL {symbol} {headline[:140]}", flush=True)
        return None
...
def _news_event_is_duplicate(item, now_ts=None):
    ...
    if not symbol or len(current_tokens) < 2:
        return False
    ...
    for old in SENT_NEWS_EVENTS:
        try:
            ...
            # Same broad event class is a strong guard against unrelated stories.
            ...
            # Same story with different wording/source normally clears one of these.
            if jaccard >= 0.48 or containment >= 0.72:
                duplicate = True
                print(
    ...
    while True:
        try:
            ...
            if not baseline_ready:
                baseline_ready = True
                print("NEWS MONITOR V11 READY — NO-NEUTRAL + STRONG DIRECTION + FRESH-ONLY + EVENT DEDUPE", flush=True)
...
async def buttons(
    ...
    if query.data == "top10":
        ...
        try:
            ...
            news_text = format_combined_news()
