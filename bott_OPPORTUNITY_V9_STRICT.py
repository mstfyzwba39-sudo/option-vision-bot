...
def build_opportunity_evaluation(symbol):
    ...
    # Live-session confirmation matters. Closed market cannot receive a "strong"
    # label solely from stale/last-session technical data.
    session_points = 12 if market_open else 0

    score = int(round(min(
        trend_points + momentum_points + participation_points + session_points,
        100
    )))
    ...
    missing = []
    if not market_open:
        missing.append("تأكيد الحركة بعد افتتاح السوق")
    if participation_points <= 0:
        missing.append("تأكيد مشاركة العقود")
    if momentum_score < 7:
        missing.append("زخم أقوى")

    if market_open and score >= 82:
        verdict = "🟢 فرصة قوية — مع تأكيد دخول"
    elif score >= 68:
    ...
    reasons.append(f"زخم {momentum_text}")
    reasons.append(
        "مشاركة العقود داعمة"
        if participation_points > 0
    ...
    return (
        ...
        f"⚡ الزخم: {momentum_text} ({momentum_score:.1f}/10)\n"
