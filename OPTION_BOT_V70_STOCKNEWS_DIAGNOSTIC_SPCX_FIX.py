...
def get_stocknews_primary_news(force=False):
    """One batched Premium call for our tracked symbols; cached to protect the trial quota."""
    ...
    if (not force and cached is not None and
        ...
        print(f"V70 STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)
    ...
    params = {
        ...
        "date": "today",
        "type": "article",
        # Official docs allow optional rank score metadata. If an account/version
        # ignores this field, the normal news payload still works.
        "extra-fields": "rankscore",
        "token": STOCK_NEWS_API_TOKEN,
    }
    try:
        r = requests.get(STOCK_NEWS_API_URL, params=params, timeout=STOCK_NEWS_API_TIMEOUT_SECONDS)
        r.raise_for_status()
        payload = r.json()
        raw_items = payload.get("data") if isinstance(payload, dict) else payload
        STOCK_NEWS_DIAG.update({"http": r.status_code, "received": len(raw_items) if isinstance(raw_items, list) else 0, "cache_hit": False, "error": None})
        print(f"V70 STOCKNEWS HTTP={r.status_code} | RECEIVED={len(raw_items) if isinstance(raw_items, list) else 0}", flush=True)
        if not isinstance(raw_items, list):
            raise ValueError("Stock News API response has no data list")
        items = []
        ...
        print(f"STOCK NEWS API PREMIUM OK: {len(items)} normalized items (batched)", flush=True)
        print(f"V70 STOCKNEWS SYMBOLS: {sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        return list(items)
    except Exception as exc:
        STOCK_NEWS_CACHE["error"] = f"{type(exc).__name__}: {exc}"
        STOCK_NEWS_DIAG.update({"error": f"{type(exc).__name__}: {exc}", "cache_hit": False})
        print(f"STOCK NEWS API PREMIUM ERROR: {type(exc).__name__}: {exc} — using fallback feeds", flush=True)
...
def format_combined_news():
    ...
    print(
        f"V70 DIAG STOCKNEWS HTTP={STOCK_NEWS_DIAG.get('http')} | "
