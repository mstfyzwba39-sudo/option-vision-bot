# V60: weekly Monday events + same-day reminders; news dedupe/source/translation hardening; no repeat until +50% across market days.
import os
import time
import math
import asyncio
...
def _load_auto_sent_state():
    try:
        ...
        return {
            ...
            "auto_contracts": dict(data.get("auto_contracts") or {}),
        }
    except Exception as exc:
...
def _save_auto_sent_state():
    try:
        payload = {
            ...
            "auto_contracts": AUTO_SENT_CONTRACTS,
        }
...
AUTO_SENT_CONTRACTS = dict(AUTO_SENT_STATE.get("auto_contracts") or {})


def _auto_contract_history_key(item):
    ...
    return "|".join([
        ...
        str(float(contract.get("strike", 0) or 0)),
        str(contract.get("expiration") or ""),
    ])


def _update_auto_contract_history(item, mark_sent=False):
    """Track first auto-send price and whether the contract ever reached +50%."""
    key = _auto_contract_history_key(item)
    if not key:
        return None
    contract = item.get("contract") or {}
    ask = float(contract.get("ask", 0) or 0)
    now_ts = time.time()
    market_day = datetime.now(ZoneInfo("America/New_York")).date().isoformat()
    row = AUTO_SENT_CONTRACTS.get(key)
    if row is None and mark_sent:
        row = {
            ...
            "first_ask": ask,
            "highest_ask": ask,
            "first_sent_at": now_ts,
            "last_sent_at": now_ts,
            "last_sent_market_day": market_day,
            "send_count": 1,
            "achieved_50": False,
        }
        AUTO_SENT_CONTRACTS[key] = row
    elif row is not None:
        if ask > 0:
            row["highest_ask"] = max(float(row.get("highest_ask", 0) or 0), ask)
            first = float(row.get("first_ask", 0) or 0)
            if first > 0 and ask >= first * 1.50:
                row["achieved_50"] = True
        if mark_sent:
            row["last_sent_at"] = now_ts
            row["last_sent_market_day"] = market_day
            row["send_count"] = int(row.get("send_count", 0) or 0) + 1
    return row


def _blocked_by_no_repeat_until_50(item):
    """Block re-sending on a later US market day until +50% was achieved."""
    row = AUTO_SENT_CONTRACTS.get(_auto_contract_history_key(item))
    if not row:
        return False
    _update_auto_contract_history(item, mark_sent=False)
    if bool(row.get("achieved_50")):
        return False
    today = datetime.now(ZoneInfo("America/New_York")).date().isoformat()
    last_day = str(row.get("last_sent_market_day") or "")
    return bool(last_day and last_day < today)


def _refresh_unresolved_auto_contract_milestones():
    """Check previously auto-sent contracts so +50% is remembered off-list too.

    Only recent unresolved contracts are checked, capped to protect API usage.
    Failures are isolated and never stop the opportunity scanner.
    """
    now_ts = time.time()
    candidates = []
    for key, row in AUTO_SENT_CONTRACTS.items():
        try:
            if row.get("achieved_50"):
                continue
            first_sent = float(row.get("first_sent_at", 0) or 0)
            if first_sent <= 0 or now_ts - first_sent > 35 * 24 * 3600:
                continue
            option_symbol = str(row.get("option_symbol") or key).strip()
            if not option_symbol:
                continue
            candidates.append((first_sent, key, option_symbol, row))
        except Exception:
            continue

    for _first_sent, key, option_symbol, row in sorted(candidates, reverse=True)[:25]:
        try:
            quote = get_option_quote_by_symbol(option_symbol)
            ...
            if ask >= first * 1.50:
                row["achieved_50"] = True
                row["achieved_50_at"] = now_ts
                print("AUTO CONTRACT +50 UNLOCKED:", row.get("symbol"), option_symbol, flush=True)
        except Exception as exc:
            print("AUTO CONTRACT MILESTONE CHECK ERROR:", option_symbol, repr(exc), flush=True)
...
_NEWS_SOURCE_PRIORITY = {
    "investor relations": 110, "official": 109, "sec": 108,
    "federal reserve": 108, "bls": 108, "bea": 108,
    "reuters": 100,
...
def format_news_results(results):
    ...
    formatted = []
    for item in _dedupe_news_results(results):
        try:
            card = format_news_item(item)
            if card:
                formatted.append(card)
        except Exception as exc:
            print("NEWS FORMAT ERROR:", repr(exc), flush=True)

    if not formatted:
        return (
            "📰 أهم الأخبار\n\n"
            "✅ لا توجد أخبار جوهرية مترجمة بوضوح حاليًا."
        )

    return (
        "📰 أهم أخبار أسهمنا\n"
        "✅ خبر واحد لكل حدث + أفضل مصدر متاح + ترجمة عربية واضحة\n"
        "━━━━━━━━━━━━━━\n\n"
        + "\n\n━━━━━━━━━━━━━━\n\n".join(formatted)
    )
...
# Automatic reminders for U.S. market events.
# V60: one weekly digest every Monday at noon KSA, then one reminder on the event day.
# Manual "أهم الأحداث" remains unchanged and always shows upcoming events on request.
...
async def monitor_major_us_events(application):
    """V60: Monday weekly digest at noon KSA + same-day reminders only.

    This worker is deliberately isolated: calendar/network errors are logged and
    never stop contracts, opportunities, or the rest of the bot.
    """
    ...
    while True:
        try:
            ...
            # Every Monday: one clean summary for the current week only.
            if now.weekday() == 0:
                monday = now.date()
                sunday = monday + timedelta(days=6)
                week_key = f"WEEKLY|{monday.isoformat()}"
                if week_key not in SENT_EVENT_REMINDERS:
                    week_events = [
                        e for e in events
                        if monday <= e["dt"].date() <= sunday and e["dt"] >= now
                    ]
                    if week_events:
                        arabic_days = {
                        ...
                        blocks = []
                        for event in sorted(week_events, key=lambda x: x["dt"]):
                            blocks.append(
                                f"{event['heat']} | {arabic_days[event['dt'].weekday()]}\n"
                                f"🇺🇸 {event['name']}\n"
                                f"🗓️ {event['dt'].strftime('%d/%m/%Y')}\n"
                        ...
                        for user_id in ALLOWED_USERS:
                            try:
                                ...
                                print("WEEKLY EVENT SEND ERROR:", user_id, send_error, flush=True)
                        if sent_any:
                            SENT_EVENT_REMINDERS.add(week_key)
                            _save_auto_sent_state()

            # Same-day reminder for every curated event that has not happened yet.
            for event in events:
                if event["dt"].date() != now.date() or event["dt"] < now:
                    continue
                key = f"{event['name']}|{event['dt'].strftime('%Y-%m-%d %H:%M')}|TODAY"
                ...
                for user_id in ALLOWED_USERS:
                    try:
                        ...
                        print("EVENT TODAY SEND ERROR:", user_id, send_error, flush=True)
                if sent_any:
...
async def monitor_auto_top_opportunities(application):
    ...
    while True:
        try:
            ...
            # Refresh +50% milestones even for contracts that temporarily dropped
            # out of the current top list. This is isolated from the main scan.
            await asyncio.to_thread(_refresh_unresolved_auto_contract_milestones)
            ...
            for item in qualified:
                ...
                if before_state is None:
                    candidate_trigger = "NEW"
                elif before_state == "YELLOW" and now_state == "GREEN":
                    candidate_trigger = "CONFIRMED"

                if candidate_trigger:
                    # V59/V60 rule: if this exact contract was auto-sent on an
                    # earlier market day, do not repeat it until it has traded
                    # at least +50% above the first auto-send ask. Same-day
                    # YELLOW→GREEN confirmation is still allowed.
                    if _blocked_by_no_repeat_until_50(item):
                        print(
                            "AUTO TOP SUPPRESSED UNTIL +50%:",
                            item.get("symbol"),
                            contract.get("option_symbol"),
                            flush=True,
                        )
                    ...
                    trigger_reason[key] = candidate_trigger
            ...
            if triggered:
                ...
                if sent_any_user:
                    for item in triggered:
                        _update_auto_contract_history(item, mark_sent=True)

            # Even when no message is sent, refresh +50% milestones for contracts
            # that are present in the current qualified scan.
            for item in qualified:
                _update_auto_contract_history(item, mark_sent=False)
...
async def post_init(application):
    ...
    # V60 U.S. events: Monday noon weekly digest + same-day reminder only.
    asyncio.create_task(monitor_major_us_events(application))
    print("WEEKLY + SAME-DAY US EVENT REMINDERS STARTED")
