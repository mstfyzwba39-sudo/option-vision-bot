...
def make_chart(symbol, resolution):
    ...
    # Conservative expected path: NEVER draw over historical candles.
    # It starts strictly to the right of the last candle and remains conditional
    # until confirmation is reached. This is a scenario guide, not a price forecast.
    path_x0 = x_last + 1.15
    trigger = setup["trigger"]
    inv = setup["invalidation"]
    if direction == "BULLISH":
        first = trigger
        pullback = trigger - max((trigger - last_price) * 0.22, avg_range * 0.35)
        path_y = [last_price, first, pullback, targets[0], targets[1], targets[2], targets[3]]
        path_color = gold
    elif direction == "BEARISH":
        first = trigger
        bounce = trigger + max((last_price - trigger) * 0.22, avg_range * 0.35)
        path_y = [last_price, first, bounce, targets[0], targets[1], targets[2], targets[3]]
        path_color = invalid
    else:
        # Neutral setup: show only the route to confirmation, then targets.
        step = 1 if trigger >= last_price else -1
        retest = trigger - step * max(abs(trigger - last_price) * 0.16, avg_range * 0.28)
        path_y = [last_price, trigger, retest, targets[0], targets[1], targets[2], targets[3]]
        path_color = gold if step > 0 else invalid

    path_x = [path_x0, path_x0 + 2.0, path_x0 + 3.0, path_x0 + 4.2,
              path_x0 + 5.4, path_x0 + 6.6, path_x0 + 7.8]
    ax.plot(path_x, path_y, color=path_color, linewidth=1.65,
            linestyle=(0, (4, 3)), alpha=0.9, zorder=2)
    path_label_offset = max(avg_range * 0.35, last_price * 0.0015)
    ax.text(path_x0 + 2.15, trigger + (path_label_offset if trigger >= last_price else -path_label_offset),
            "EXPECTED PATH", color=charcoal, fontsize=10.5, fontweight="bold",
            va="bottom" if trigger >= last_price else "top",
            bbox={"boxstyle": "round,pad=0.14", "facecolor": "#FFFDF8", "edgecolor": "#D7C9A4", "linewidth": 0.6},
            zorder=6)
