...
def _detect_clean_setup(highs, lows, closes):
    ...
    def _detect_demand_zone():
        ...
        if len(pivots) < 2:
            return None
        tolerance = max(atr * 0.65, last_price * 0.0045)
        defended = []
        for i, p in pivots:
            after = c[i+1:min(len(c), i+8)]
            if after and max(after) - p >= atr * 1.15:
                defended.append((i, p))
        best = None
        for _, anchor in defended:
            cluster = [(i,p) for i,p in defended if abs(p-anchor) <= tolerance]
            if len(cluster) < 2:
                continue
            prices = [p for _,p in cluster]
            lower = min(prices) - atr*0.12
            upper = max(prices) + atr*0.32
            if upper >= last_price + atr*0.20:
                continue
            score = len(cluster)*25 + max(i for i,_ in cluster)
    ...
    if candidates:
        # Prefer the strongest valid pattern; score already penalizes weak geometry.
...
def make_chart(symbol, resolution):
    ...
    if chart_targets:
        ...
        ax.plot(
            points_x, points_y, color=path_color, linewidth=4.2, alpha=0.92,
            solid_capstyle="round", solid_joinstyle="round", zorder=5,
        )
        ax.scatter(points_x, points_y, s=34, color=path_color, zorder=6)
        ax.annotate(
            "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
            arrowprops={"arrowstyle": "-|>", "color": path_color, "lw": 4.2, "alpha": 0.96},
            zorder=6,
        )
        label = "EXPECTED PATH" if direction != "NEUTRAL" else "PATH AFTER CONFIRM"
        ax.text(
            points_x[1], points_y[1], label, color=path_color, fontsize=13.5,
