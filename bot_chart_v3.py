...
def make_chart(symbol, resolution):
    ...
    bearish_candle = "#252525"
    wick_color = "#2E2E2E"
    gold = "#B08A3E"
    gold_soft = "#D8C497"
    charcoal = "#333333"
    muted = "#8B857B"
    invalidation_color = "#806F5A"
    ...
    if setup["type"] == "triangle":
        ...
        ax.plot(chart_x, upper_y, color=muted, linewidth=0.9, alpha=0.72, zorder=2)
        ax.plot(chart_x, lower_y, color=muted, linewidth=0.9, alpha=0.72, zorder=2)
    else:
        structure_start = max(0, len(closes) - 18)
        ax.hlines(
            setup["resistance"], structure_start, future_end,
            color="#B7B2A9", linewidth=0.75, linestyle=":", alpha=0.62, zorder=1,
        )
        ax.hlines(
            setup["support"], structure_start, future_end,
            color="#B7B2A9", linewidth=0.75, linestyle=":", alpha=0.62, zorder=1,
        )
    ...
    # Keep only the first three targets on the image to avoid clutter.
    all_targets = setup["targets"][:4]
    chart_targets = all_targets[:3]
    ...
    # One directional path only. No forecast spaghetti.
    if direction in ("BULLISH", "BEARISH") and chart_targets:
        path_target = chart_targets[min(1, len(chart_targets) - 1)]
        ax.annotate(
            "",
            xy=(x_last + 6.3, path_target),
            xytext=(x_last + 0.6, last_price),
            arrowprops={
                "arrowstyle": "-|>",
                "color": level_color,
                "lw": 1.5,
    ...
    ax.tick_params(axis="y", colors="#77736C", labelsize=8.2, length=0, pad=4)
