...
def format_combined_news():
    ...
    try:
        stock_items = scan_manual_stock_news()
        if not stock_items:
            print("V66 MANUAL STOCK NEWS: strict Finnhub + Google RSS scan returned no material company stories", flush=True)
    except Exception as exc:
        print(f"MANUAL STOCK NEWS SCAN ERROR: {type(exc).__name__}: {exc}", flush=True)
        stock_items = NEWS_CACHE.get("results") or _load_verified_daily_news()
