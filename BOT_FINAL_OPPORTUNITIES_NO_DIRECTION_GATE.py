...
def refresh_top10_contract_prices(results):
    """Refresh only the winning contracts before display/watch.

    scan_top10 may legitimately come from the 15-minute analysis cache. During
    market hours we do NOT want Bid/Ask/Volume shown to the user to come from
    that cache, so this makes a fresh exact-expiry request for each displayed
    winner and updates market-sensitive contract fields.
    """
    if not results or not is_us_market_open():
        return results
    ...
    for item in refreshed_results:
        try:
            ...
            expiry = datetime.fromtimestamp(
                int(expiration), ZoneInfo("America/New_York")
            ).date()

            fresh = get_exact_option_contract(
                symbol,
                str(contract.get("side", "")).upper(),
                float(contract.get("strike", 0)),
                expiry,
            )
            if not fresh:
                print("PRICE REFRESH MISS:", symbol, contract.get("option_symbol"), flush=True)
                continue

            # Guard against unusable quotes. If fresh quote is invalid, retain
            # the scanned value rather than displaying a misleading zero.
            if fresh.get("ask", 0) <= 0 or fresh.get("bid", 0) < 0:
                print("PRICE REFRESH INVALID:", symbol, fresh, flush=True)
                continue

            contract["bid"] = fresh["bid"]
            contract["ask"] = fresh["ask"]
            contract["mid"] = fresh["mid"]
            contract["volume"] = fresh["volume"]
            contract["oi"] = fresh["oi"]
            contract["delta"] = fresh["delta"]
            contract["spread_pct"] = fresh["spread_pct"]

            # Recalculate displayed contract quality from the refreshed market
            # fields while preserving the existing trend adjustment logic.
            raw_score = contract_score(
                contract["delta"], contract["volume"], contract["oi"],
                contract["spread_pct"], contract["dte"]
            )
            contract["base_score"] = normalize_contract_score(raw_score)
            ...
            contract["internal_score"] = (
                contract["base_score"]
                + contract.get("market_adjustment", 0)
                + contract["uoa_adjustment"]
            )
            contract["score"] = max(0, min(round(contract["internal_score"]), 98))
            item["internal_score"] = contract["internal_score"]

            print(
                "PRICE REFRESH OK:", symbol, contract.get("option_symbol"),
                "bid=", contract["bid"], "ask=", contract["ask"],
                "vol=", contract["volume"], flush=True
            )
        except Exception as exc:
...
def format_top10(results):
    ...
    lines = [
        ...
        "🕒 السوق مفتوح — تأكيد 15د لكل فرصة" if market_open else "🕒 السوق مغلق — تأكيد 15د بعد الافتتاح",
        "📍 🟢 مناسب | 🟡 قريب | ⏳ بعيد",
        "━━━━━━━━━━━━",
        "",
    ]
