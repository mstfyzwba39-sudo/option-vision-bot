...
def get_option_chain(symbol):
    ...
    # Fallback changes ONLY the chain request filters.
    # Scoring, ranking, news, charts, opportunity evaluation, etc. are untouched.
    fallback_params = {
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 404:
                    ...
                    # Only after the primary 404 do we try lighter filters.
                    break
                ...
                if response.status_code == 429:
                    ...
                    raise ValueError(
                        "تعذر جلب سلسلة العقود بسبب حد بيانات السوق. "
                        "جرّبي بعد قليل."
                    )
                ...
                if data.get("s") != "ok":
                    ...
                    # Let fallback try if the primary response has no usable chain.
                    if request_stage == "primary":
                        break
                    raise ValueError(
                        data.get(
                            "errmsg",
    ...
    if last_error:
        raise ValueError(
            "تعذر الاتصال ببيانات السوق بعد عدة محاولات."
        ) from last_error

    raise ValueError(
        f"لا توجد بيانات أوبشن متاحة للرمز {symbol} حاليًا "
        "حتى بعد المحاولة الاحتياطية."
    )
...
def score_news_item(symbol, item):
    ...
    if not _direct_match(symbol, item, combined):
        return None
    ...
    # Commentary/technical analysis is not treated as breaking news.
    if any(x in combined for x in ANALYSIS_ONLY_WORDS):
        return None
...
def decision_status(contract, trend):
    ...
    if contract["internal_score"] < MIN_TOP_SCORE:
        return {
            "label": "🟡 غير مؤهل للمراقبة",
            "reason": (
                "التركيبة الكلية للفرصة "
                f"{round(contract['internal_score'])}/100 "
...
def get_top_contracts(data, trend):
    ...
    for i in range(count):
        try:
            ...
            # One canonical contract-quality score everywhere.
            # Market direction and unusual activity remain separate ranking signals.
...
# =========================================================
# ENTRY ZONE STATUS — UNDERLYING STOCK
# =========================================================

def get_entry_zone_status(symbol, bias):
    """
    Entry zone is based on the UNDERLYING stock price, not the option price.
    Uses the same clean supply/demand structure as the approved chart,
    on the 1-hour timeframe. 15-minute confirmation remains separate.
    """
    try:
        data = get_chart_data(symbol, "60")
        ...
        if bias == "CALL":
            ...
            if zone_low <= last_price <= zone_high:
                status = "INSIDE"
                label = "🟢 داخل منطقة الدخول"

            elif last_price > zone_high:
                distance_pct = (
                    (last_price - zone_high)
                    / last_price
                    * 100
                )

                if distance_pct <= 1.25:
                    status = "NEAR"
                    label = "🟡 قريب من منطقة الدخول"
                else:
                    status = "WAIT"
                    label = "⏳ بانتظار رجوع السعر للمنطقة"

            else:
                status = "INVALID"
                label = "🔴 أسفل منطقة الدخول"
            ...
            if zone_low <= last_price <= zone_high:
                ...
                if distance_pct <= 1.25:
                    ...
                    status = "WAIT"
                    label = "⏳ بانتظار صعود السعر للمنطقة"

            else:
                status = "INVALID"
                label = "🔴 أعلى منطقة الدخول"

        else:
            return {
                "status": "UNKNOWN",
                "label": "⚪ الاتجاه غير محسوم",
                "zone_text": "—",
            }
...
def scan_top10():
    ...
    if cached is not None and (now - TOP10_CACHE["time"]) < CACHE_SECONDS:
        return cached
    ...
    for symbol in SCAN_SYMBOLS:
        try:
            ...
            entry_zone = get_entry_zone_status(
                symbol,
                trend["bias"]
            )

            # Keep only technically valid contracts.
            # Do NOT require the opportunity to already be confirmed;
            # ranking/status will tell the user how strong it is.
            matching = [
                contract
                for contract in result["contracts"]
                if (
                    contract["side"] == trend["bias"]
                    and contract["score"] >= 70
            ...
            if metrics["market_open"]:
                if metrics["live_status"] == "CONFIRMED" and metrics["score"] >= 82:
                    opportunity_label = "🟢 فرصة مؤكدة"
                elif metrics["live_status"] == "INVALID":
                    # Do not include a setup actively invalidated by live action.
                    continue
                else:
                    opportunity_label = "🟡 انتظار تأكيد"
            else:
                opportunity_label = "👀 تحت المراقبة — انتظار افتتاح السوق"

            result["contract"] = contract
            result["internal_score"] = contract["internal_score"]
            result["decision"] = effective_decision(contract, trend)
            result["opportunity_score"] = metrics["score"]
            result["opportunity_label"] = opportunity_label
            result["market_open"] = metrics["market_open"]
    ...
    # Rank the valid candidates, then show up to 10.
    results.sort(
        key=lambda x: (
            -x["opportunity_score"],
            -x["internal_score"],
...
def format_top10(results):
    if not results:
        return (
            "🏆 أفضل فرص اليوم\n\n"
            "❌ لا توجد فرص تحقق الشروط حاليًا."
        )

    market_open = bool(results[0].get("market_open"))
    if market_open:
        title = f"🏆 أفضل {len(results)} فرص لليوم"
        note = "🟢 يشترط تأكيد اتجاه السهم حيًا على 15 دقيقة"
    else:
        title = f"👀 أفضل {len(results)} فرص تحت المراقبة"
        note = "🕒 السوق مغلق — لا تعتبر دخولًا حتى تأكيد الحركة بعد الافتتاح"
    ...
    for index, item in enumerate(results, start=1):
        ...
        message += (
            f"{index}️⃣ {item['symbol']} "
            f"{contract['side']} {contract['strike']:g} | "
            f"{contract['expiry_date']}\n"
            f"{item['opportunity_label']}\n"
            f"🎯 قوة فرصة السهم {item['opportunity_score']}/100\n"
            f"📍 منطقة الدخول: "
            f"{item['entry_zone']['label']} "
            f"({item['entry_zone']['zone_text']})\n"
            f"⏱️ تأكيد السوق: "
            f"{'🕒 مغلق — ننتظر 15 دقيقة بعد الافتتاح' if not item['market_open'] else 'يعتمد على حركة 15 دقيقة الحية'}\n"
            f"⭐ جودة العقد {contract['score']}/100 | "
...
def format_top_contracts(
    ...
    qualified = [
        ...
        if (
            contract["side"] == trend["bias"]
            and contract["internal_score"] >= MIN_TOP_SCORE
            and contract["uoa_score"] >= MIN_TOP_UOA
            and contract["volume"] >= MIN_VOLUME
            and contract["decision"]["rank"] > 0
        )
...
def opportunity_metrics(symbol, trend=None, contracts=None):
    ...
    same_side = [
        ...
        if str(c.get("side", "")).upper() == bias
    ]
    if same_side:
        best_score = max(float(c.get("score", 0) or 0) for c in same_side)
    ...
    if market_open and bias in ("CALL", "PUT"):
        try:
            ...
            status, _ = check_intraday_confirmation(bias, intraday)
            live_status = status
            if status == "CONFIRMED":
                live_points = 12
            elif status == "INVALID":
    ...
    score = int(round(max(0, min(
        trend_points + momentum_points + participation_points + live_points,
        100
    ))))
