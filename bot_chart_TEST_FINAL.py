...
def _detect_clean_setup(highs, lows, closes):
    """Detect only a real converging triangle; otherwise return plain structure.

    Triangle detection uses recent swing highs/lows rather than fitting every candle,
    which avoids inventing a pattern from noisy price action.
    """
    last_price = closes[-1]
    atr = calculate_atr(highs, lows, closes, 14)
    n = len(closes)
    ...
    best_triangle = None
    for lookback in (18, 22, 26, 30, 35, 40):
        if n < lookback:
            continue
        h = highs[-lookback:]
        l = lows[-lookback:]
        ...
        # Use recent meaningful swings only.
        ph = ph[-4:]
        pl = pl[-4:]
        ...
        upper_start = us * start_x + ui
        lower_start = ls * start_x + li
        upper_end = us * end_x + ui
        lower_end = ls * end_x + li
        start_gap = upper_start - lower_start
        end_gap = upper_end - lower_end
        if start_gap <= 0 or end_gap <= 0 or end_gap >= start_gap:
            continue

        convergence = 1.0 - end_gap / start_gap
        # Symmetrical converging triangle: descending highs + rising lows.
        if not (us < 0 and ls > 0):
            continue
        if convergence < 0.18:
            continue
        if ur2 < 0.45 or lr2 < 0.45:
            continue

        # Price should still be reasonably inside/near the triangle at the right edge.
        tolerance = max(atr * 0.45, last_price * 0.003)
        if last_price > upper_end + tolerance or last_price < lower_end - tolerance:
            continue

        score = convergence * 100 + (ur2 + lr2) * 20
    ...
    # Demand zone: require repeated defended lows + a real bounce.
    # If the evidence is weak, return None and draw nothing.
    def _detect_demand_zone():
        ...
        tolerance = max(atr * 0.55, last_price * 0.004)
        candidates = []
        for i, price in pivots:
            # A defended low must be followed by a meaningful bounce.
            after = c[i + 1:min(len(c), i + 7)]
            if not after or max(after) - price < atr * 1.10:
                continue
        ...
        best = None
        for anchor_i, anchor_p in reversed(candidates):
            cluster = [(i, p) for i, p in candidates if abs(p - anchor_p) <= tolerance]
            if len(cluster) < 2:
                continue
            prices = [p for _, p in cluster]
            lower = min(prices) - atr * 0.10
            upper = max(prices) + atr * 0.28
            # Demand must sit below/around current price, not above it.
            if lower >= last_price or upper > last_price + atr * 0.25:
                continue
            recency = max(i for i, _ in cluster)
            score = len(cluster) * 20 + recency
            if best is None or score > best["score"]:
                best = {"lower": lower, "upper": upper, "touches": len(cluster), "score": score}
        return best
    ...
    if best_triangle:
        ...
        x_now = lookback - 1
        upper_now = _line_value(best_triangle["upper_slope"], best_triangle["upper_intercept"], x_now)
        lower_now = _line_value(best_triangle["lower_slope"], best_triangle["lower_intercept"], x_now)
        midpoint = (upper_now + lower_now) / 2

        bullish_bias = last_price >= midpoint and sma10 >= sma20 and trend_pct >= -0.002
        bearish_bias = last_price < midpoint and sma10 < sma20 and trend_pct <= 0.002
        ...
        if bullish_bias:
            ...
            targets = [trigger + measured * x for x in (0.30, 0.50, 0.72, 1.00)]
        elif bearish_bias:
            direction = "BEARISH"
            ...
            targets = [upper_now + measured * x for x in (0.30, 0.50, 0.72, 1.00)]

        confidence = int(min(92, max(62, 55 + best_triangle["score"] * 0.30)))
        return {
            "type": "triangle",
            "name_ar": "مثلث متقارب",
            "direction": direction,
    ...
    # No valid chart pattern: do not invent one.
...
def make_chart(symbol, resolution):
    ...
    # V3: analyst-style chart — white canvas, black/white candles and restrained gold levels.
    fig, ax = plt.subplots(figsize=(12.6, 7.2))
    fig.patch.set_facecolor("#FFFFFF")
    ax.set_facecolor("#FFFFFF")

    candle_width = 0.56
    bullish_candle = "#EAF7EF"
    bearish_candle = "#FDECEC"
    wick_color = "#444444"
    gold = "#C99A2E"
    gold_soft = "#E6D2A4"
    green = "#2E8B57"
    ...
    for i in range(len(closes)):
        ...
        rectangle = Rectangle(
            ...
            edgecolor=(green if is_up else red),
            linewidth=1.0,
            zorder=4,
        )
    ...
    future_bars = 20
    future_end = len(closes) - 1 + future_bars
    x_last = len(closes) - 1

    # Price-action structure. Keep it subtle so the candles stay dominant.
    if setup["type"] == "triangle":
        lookback = setup["lookback"]
        x_start = len(closes) - lookback
        local_x = list(range(lookback + future_bars // 2))
        chart_x = [x_start + x for x in local_x]
        upper_y = [
            _line_value(setup["upper_slope"], setup["upper_intercept"], x)
            for x in local_x
        ]
        lower_y = [
            _line_value(setup["lower_slope"], setup["lower_intercept"], x)
            for x in local_x
        ]
        ax.plot(chart_x, upper_y, color=gold, linewidth=1.6, alpha=0.88, zorder=2)
        ax.plot(chart_x, lower_y, color=gold, linewidth=1.6, alpha=0.88, zorder=2)
    ...
    if demand_zone:
        ...
        demand_start = max(0, x_last - 22)
        ax.axhspan(
            demand_lower, demand_upper,
            xmin=max(0.0, demand_start / max(1, future_end + 8.5)),
            xmax=min(1.0, (future_end + 0.5) / max(1, future_end + 8.5)),
            facecolor=gold_soft, alpha=0.22, zorder=0,
        )
        ax.text(
            demand_start + 0.5, demand_upper,
            f"DEMAND  {demand_lower:.2f}–{demand_upper:.2f}",
            color="#9A741B", fontsize=9.8, fontweight="bold", va="bottom", zorder=2,
        )
    ...
    # Expected path: one stepped scenario based on structure, confirmation,
    # demand/retest when available, and the four calculated targets.
    if chart_targets:
        path_color = green if direction == "BULLISH" else red if direction == "BEARISH" else gold
        demand_zone = setup.get("demand_zone")
        points_y = [last_price]

        # For bullish/neutral scenarios, a valid demand zone can be the first retest.
        if direction != "BEARISH" and demand_zone:
            demand_mid = (demand_zone["lower"] + demand_zone["upper"]) / 2
            if setup["invalidation"] < demand_mid < last_price:
                points_y.append(demand_mid)

        # Confirmation is the gate before targets.
        points_y.append(setup["trigger"])

        if direction == "BEARISH":
            points_y.extend([
                chart_targets[0],
                (chart_targets[0] + setup["trigger"]) / 2,
                chart_targets[1] if len(chart_targets) > 1 else chart_targets[0],
                chart_targets[2] if len(chart_targets) > 2 else chart_targets[-1],
                chart_targets[3] if len(chart_targets) > 3 else chart_targets[-1],
            ])
        else:
            # Breakout -> T1 -> small retest -> T2 -> T3 -> T4.
            points_y.extend([
                chart_targets[0],
                (chart_targets[0] + setup["trigger"]) / 2,
                chart_targets[1] if len(chart_targets) > 1 else chart_targets[0],
        ...
        # Spread the path across the future area; it is a scenario, not a time forecast.
        step = 2.45
        points_x = [x_last + 0.45 + i * step for i in range(len(points_y))]
        ax.plot(
            points_x, points_y, color=path_color, linewidth=2.6, alpha=0.82,
            solid_capstyle="round", solid_joinstyle="round", zorder=5,
        )
        ax.annotate(
            "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
            arrowprops={"arrowstyle": "-|>", "color": path_color, "lw": 2.6, "alpha": 0.92},
            zorder=6,
        )
        label = "EXPECTED PATH" if direction != "NEUTRAL" else "PATH AFTER CONFIRM"
        ax.text(
            points_x[1], points_y[1], label, color=path_color, fontsize=9.5,
    ...
    ax.tick_params(axis="y", colors="#77736C", labelsize=9.4, length=0, pad=4)
    ax.tick_params(axis="x", labelbottom=False, length=0)
    ax.grid(False)

    for side in ("top", "left", "bottom"):
        ax.spines[side].set_visible(False)
    ax.spines["right"].set_color("#E5E1D9")
    ...
    caption = (
        ...
        + (f"منطقة الطلب: ${setup['demand_zone']['lower']:.2f} - ${setup['demand_zone']['upper']:.2f}\n" if setup.get("demand_zone") else "منطقة الطلب: لا توجد منطقة موثوقة حاليًا\n")
