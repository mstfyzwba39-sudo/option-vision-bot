# OPTION_BOT V64 - quality news filtering, trusted sources, stale-story guard, visible source links
import os
import time
import math
import asyncio
...
def _v67_finalize_stock_news(items):
    ...
    cleaned.sort(key=lambda x:(-int(x.get("importance") or 0), -_news_source_rank(x), -int(x.get("timestamp") or 0)))
...
def format_combined_news():
    ...
    stock_items.sort(key=lambda x: (-x.get("importance", 0), -_news_source_rank(x), -x.get("timestamp", 0)))
    ...
    while True:
        try:
            ...
            if not baseline_ready:
                ...
                baseline_ready = True
                print(
                    f"NEWS MONITOR V47 READY — BASELINED {baselined} CURRENT ITEMS; "
