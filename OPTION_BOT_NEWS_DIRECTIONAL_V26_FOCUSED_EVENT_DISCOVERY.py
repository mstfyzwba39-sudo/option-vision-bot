...
def _material_query_for_symbol(symbol):
    ...
    material_terms = [
        'recall', 'recalled', 'recalling', 'investigation', 'probe', 'regulator',
        'lawsuit', 'acquisition', 'merger', 'guidance', '"profit warning"',
        '"CEO resigns"', '"CEO steps down"', '"FDA approval"', '"FDA rejects"',
    ]

    # V26 focused discovery: these were being accepted by scoring only IF another
    # feed happened to surface them. Add them to the actual discovery query too.
    if str(symbol).upper() == "TSLA":
        material_terms += [
            '"Cybercab"', '"robotaxi"', '"Optimus"',
            '"launch event"', '"launch date"', '"event date"',
            '"sends out invites"', '"sending invitations"', '"confirms date"',
            '"official date"', '"scheduled for"',
        ]
    elif str(symbol).upper() == "SPCX":
        material_terms += [
            '"Starship"', '"launch event"', '"launch date"', '"event date"',
            '"confirms date"', '"official date"', '"scheduled for"',
        ]
    elif str(symbol).upper() == "BA":
        material_terms += [
            '"union rejects contract"', '"union rejects offer"',
            '"authorizes strike"', '"strike authorization"',
            '"vote to strike"', '"workers reject contract"',
            '"engineers reject contract"',
        ]

    material = " OR ".join(material_terms)
...
def score_news_item(symbol, item):
    ...
    if pos >= 1 and pos > neg:
        ...
        reason == "إعلان موعد إطلاق أو حدث جوهري"
        and any(x in combined for x in major_launch_products)
        and any(x in combined for x in major_launch_event_markers)
    ):
        # A confirmed launch/event date is a company catalyst. In this bot's
        # directional-only mode, treat a clean confirmation as positive rather
        # than dropping it as "neutral".
        sentiment, sentiment_ar = "POSITIVE", "🟢 إيجابي"
    else:
