...
# =========================================================
# UPCOMING US ECONOMIC EVENTS — OFFICIAL CALENDARS
# =========================================================

UPCOMING_EVENT_DAYS = 7
MAJOR_EVENT_LOOKAHEAD_DAYS = 45

IMPORTANT_EVENT_NAMES = {
    "Consumer Price Index": ("CPI — مؤشر أسعار المستهلك", "🔥🔥🔥"),
    "Producer Price Index": ("PPI — مؤشر أسعار المنتجين", "🔥🔥🔥"),
    "Employment Situation": ("الوظائف والبطالة الأمريكية", "🔥🔥🔥"),
    "Job Openings and Labor Turnover Survey": ("JOLTS — فرص العمل", "🔥🔥"),
    "Gross Domestic Product": ("GDP — الناتج المحلي", "🔥🔥🔥"),
    "GDP": ("GDP — الناتج المحلي", "🔥🔥🔥"),
    "Personal Income and Outlays": ("PCE — الدخل والإنفاق والتضخم", "🔥🔥🔥"),
    "Retail Sales": ("مبيعات التجزئة الأمريكية", "🔥🔥🔥"),
}

# Known 2026 FOMC decision dates. Decision time is 2:00 PM New York time.
FOMC_2026_DATES = [
...
def _parse_official_date_time(date_text, time_text):
    # Official BLS/BEA calendars publish in US Eastern time.
    date_text = " ".join(date_text.replace(",", " ").split())
    ...
    tm = time_text.strip().upper().replace(".", "")
    try:
        tt = datetime.strptime(tm, "%I:%M %p")
    except ValueError:
        return None

    eastern = datetime(
        d.year, d.month, d.day, tt.hour, tt.minute,
        tzinfo=ZoneInfo("America/New_York")
    )
    return eastern.astimezone(ZoneInfo("Asia/Riyadh"))


def _fetch_bls_upcoming():
    ...
    # Match official BLS text: weekday, Month DD, YYYY HH:MM AM Release...
    pattern = re.compile(
        rf"(Monday|Tuesday|Wednesday|Thursday|Friday),\s+"
        rf"({month_names})\s+(\d{{1,2}}),\s+({year})\s+"
        rf"(\d{{1,2}}:\d{{2}}\s+[AP]M)\s+(.{{0,170}}?)"
        rf"(?=(?:Monday|Tuesday|Wednesday|Thursday|Friday),\s+(?:{month_names})|\Z)"
    )
    for m in pattern.finditer(page):
        date_text = f"{m.group(2)} {m.group(3)} {m.group(4)}"
        time_text = m.group(5)
        release_text = m.group(6).strip()

        matched = None
        for key, (arabic_name, heat) in IMPORTANT_EVENT_NAMES.items():
            if key.lower() in release_text.lower():
                matched = (arabic_name, heat)
                break
        if not matched:
            continue

        dt = _parse_official_date_time(date_text, time_text)
        if dt:
            events.append({
                "name": matched[0], "heat": matched[1],
                "dt": dt, "source": "BLS"
            })
    return events


def _fetch_bea_upcoming():
    r = requests.get(
    ...
    pattern = re.compile(
        rf"({month_names})\s+(\d{{1,2}})\s+(\d{{1,2}}:\d{{2}}\s+[AP]M)\s+"
        rf"(.{{0,190}}?)(?=(?:{month_names})\s+\d{{1,2}}\s+\d{{1,2}}:\d{{2}}\s+[AP]M|\Z)"
    )
    for m in pattern.finditer(page):
        title = m.group(4).strip()
        matched = None
        if "GDP" in title:
            matched = ("GDP — الناتج المحلي", "🔥🔥🔥")
        elif "Personal Income and Outlays" in title:
            matched = ("PCE — الدخل والإنفاق والتضخم", "🔥🔥🔥")
        if not matched:
            continue
        dt = _parse_official_date_time(
...
def _fomc_upcoming():
    events = []
    for ds in FOMC_2026_DATES:
        d = datetime.strptime(ds, "%Y-%m-%d")
        eastern = datetime(
            d.year, d.month, d.day, 14, 0,
            tzinfo=ZoneInfo("America/New_York")
        )
        events.append({
...
# Major dates that should always be shown even when outside the 7-day window.
# Official 2026 schedules: BLS/Federal Reserve.
MAJOR_FIXED_2026 = [
...
def _major_fixed_upcoming():
    now = datetime.now(ZoneInfo("Asia/Riyadh"))
    end = now + timedelta(days=MAJOR_EVENT_LOOKAHEAD_DAYS)
    events = []
    for ds, tm, name, heat, source in MAJOR_FIXED_2026:
        hh, mm = map(int, tm.split(":"))
        d = datetime.strptime(ds, "%Y-%m-%d")
        eastern = datetime(d.year, d.month, d.day, hh, mm, tzinfo=ZoneInfo("America/New_York"))
        dt = eastern.astimezone(ZoneInfo("Asia/Riyadh"))
        if now <= dt <= end:
            events.append({"name": name, "heat": heat, "dt": dt, "source": source, "major": True})
    return events

def get_upcoming_us_events():
    now = datetime.now(ZoneInfo("Asia/Riyadh"))
    end = now + timedelta(days=UPCOMING_EVENT_DAYS)
    events = []

    for loader in (_fetch_bls_upcoming, _fetch_bea_upcoming):
        try:
            events.extend(loader())
        except Exception as exc:
            print("ECON CALENDAR ERROR:", exc)

    events.extend(_fomc_upcoming())
    events.extend(_major_fixed_upcoming())

    # Deduplicate. Ordinary events: 7 days. Major events: always show next one within 45 days.
    unique = {}
    for x in events:
        allowed = (now <= x["dt"] <= end) or (x.get("major") and now <= x["dt"] <= now + timedelta(days=MAJOR_EVENT_LOOKAHEAD_DAYS))
        if allowed:
            key = (x["name"], x["dt"].strftime("%Y-%m-%d %H:%M"))
            unique[key] = x
...
def format_upcoming_us_events():
    events = get_upcoming_us_events()
    if not events:
        return (
            "📅 الأحداث الأمريكية القادمة — 7 أيام\n\n"
    ...
    blocks = []
    for x in events:
        status = _riyadh_event_status(x["dt"]) or "📅 قادم"
        day = arabic_days[x["dt"].weekday()]
        blocks.append(
            f"{status} | {x['heat']}\n"
            f"🇺🇸 {x['name']}\n"
            f"🗓️ {day} {x['dt'].strftime('%d/%m/%Y')}\n"
            f"⏰ {x['dt'].strftime('%I:%M %p')} بتوقيت السعودية\n"
            f"🏛️ المصدر: {x['source']}"
        )
    return (
