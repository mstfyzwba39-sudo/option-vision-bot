...
def make_chart(symbol, resolution):
    ...
    candle_count = min(60, len(closes))
    ...
    # Clean analyst-style chart: candles + structure + trigger + invalidation + targets.
    fig, ax = plt.subplots(figsize=(11.5, 6.4))
    fig.patch.set_facecolor("#FBF8EA")
    ax.set_facecolor("#FBF8EA")

    candle_width = 0.54
    bullish_candle = "#F7F3E8"
    bearish_candle = "#9A3412"
    wick_color = "#252525"
    ...
    for i in range(len(closes)):
        ...
        rectangle = Rectangle(
            ...
            edgecolor=wick_color if is_up else bearish_candle,
            linewidth=0.9,
            zorder=4,
        )
    ...
    if setup["type"] == "triangle":
        ...
        ax.plot(chart_x, upper_y, color="#666666", linewidth=1.0, alpha=0.85, zorder=2)
        ax.plot(chart_x, lower_y, color="#666666", linewidth=1.0, alpha=0.85, zorder=2)
    else:
        structure_start = max(0, len(closes) - 20)
        ax.hlines(
            setup["resistance"],
            structure_start,
            future_end,
    ...
    target_color = "#4D8B67" if direction != "BEARISH" else "#A3543A"
    invalidation_color = "#B65A46"
    neutral_color = "#777264"

    # Trigger / confirmation.
    trigger_label = "تأكيد الاختراق" if direction == "BULLISH" else (
        "تأكيد الكسر" if direction == "BEARISH" else "مستوى التأكيد"
    )
    ax.hlines(
        setup["trigger"],
        x_last - 4,
        future_end + 1,
        color=target_color if direction != "NEUTRAL" else neutral_color,
        linewidth=1.0,
        linestyle="--",
        alpha=0.80,
        zorder=2,
    ...
    ax.text(
        future_end + 1.2,
        setup["trigger"],
        f"{setup['trigger']:.2f}  {trigger_label}",
        color=target_color if direction != "NEUTRAL" else neutral_color,
        fontsize=9,
        va="center",
        fontweight="bold",
    ...
    # Targets: 2-4 depending on available room; values always come from live market data.
    ...
    # One subtle path arrow only — no forecast spaghetti.
    if direction in ("BULLISH", "BEARISH"):
        path_target = targets[min(1, len(targets) - 1)]
        ax.annotate(
            "",
            xy=(x_last + 7.5, path_target),
            xytext=(x_last + 0.7, last_price),
            arrowprops={
                "arrowstyle": "-|>",
                "color": target_color,
                "lw": 1.8,
    ...
    ax.set_title(
        f"{symbol}   |   {timeframe}   |   ${last_price:.2f}   |   {setup['name_ar']} - {title_direction}",
        fontsize=12.5,
        fontweight="bold",
        color="#2B2A27",
        pad=12,
    ...
    # Minimal axes, like the analyst's chart.
    ax.yaxis.tick_right()
    ax.yaxis.set_label_position("right")
    ax.tick_params(axis="y", colors="#5B5952", labelsize=8.5, length=0)
    ax.tick_params(axis="x", labelbottom=False, length=0)
    ax.grid(False)

    for side in ("top", "left", "bottom"):
        ax.spines[side].set_visible(False)
    ax.spines["right"].set_color("#D9D2BF")
    ax.spines["right"].set_linewidth(0.7)

    all_y = highs + lows + [setup["trigger"], setup["invalidation"]] + targets
    y_min = min(all_y)
    y_max = max(all_y)
    padding = max((y_max - y_min) * 0.08, last_price * 0.01)

    ax.set_xlim(-1, future_end + 8)
