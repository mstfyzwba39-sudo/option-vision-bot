...
def format_top10(results):
    ...
    for idx, item in enumerate(results, 1):
        ...
        entry_text = f"{icon} {LRI}{zone_text}{PDI}" if zone_text != "—" else f"{icon} —"

        lines.extend([
            f"{idx}️⃣ {item['symbol']} {contract['side']} {contract['strike']:g} | {format_expiry_timestamp(contract['expiration'])}",
            f"🎯 {LRI}{item['opportunity_score']}%{PDI} قوة فرصة السهم",
            f"⭐ {LRI}{contract['score']}%{PDI} جودة العقد",
            f"🔥 {LRI}{contract['uoa_score']}/10{PDI} النشاط",
            entry_text,
            f"💵 {LRI}${contract['ask']:.2f}{PDI} | 📊 {LRI}{contract['volume']:,}{PDI}",
            "",
            "━━━━━━━━━━━━",
            "",
        ])
