...
def refresh_top10_contract_prices(results, allow_spx_stale_monitoring=False):
    ...
    For manual SPX scans outside the regular cash session, a stale/unknown exact
    timestamp must not make an otherwise valid candidate disappear. In that
    case we keep the contract as YELLOW monitoring-only, while regular/actionable
    opportunities still require the normal live-quote freshness guard.
    """
    if not results:
        return results
    ...
    for item in refreshed_results:
        try:
            ...
            if not usable_now:
                ...
                # Manual SPX button: outside regular U.S. cash hours, preserve a
                # basically valid candidate even when the exact quote timestamp
                # is stale/missing. It is explicitly monitoring-only, never a
                # green/actionable confirmation.
                if (
                    allow_spx_stale_monitoring
                    and symbol == "SPX"
                    and not is_us_market_open()
                    and basic_price_ok
                ):
                    item["spx_stale_monitoring"] = True
                    item.setdefault("decision", {})["state"] = "YELLOW"
                    item["decision"]["label"] = "🟡 للمراقبة فقط — السعر غير لحظي"
                    print(
                        "SPX STALE QUOTE KEPT FOR MONITORING:",
                        contract.get("option_symbol"),
    ...
    while True:
        try:
            ...
            for item in qualified:
                ...
                # If this exact contract was ever auto-sent and has not yet
                # achieved +50%, suppress it even on later days.
                    ...
                    try:
                        ...
                        for item in triggered:
                            ...
                            if option_symbol0:
                                ...
                                if not old or bool(old.get("profit_50_reached")):
