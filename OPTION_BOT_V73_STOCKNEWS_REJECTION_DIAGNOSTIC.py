...
def get_stocknews_primary_news(force=False):
    ...
    if not STOCK_NEWS_API_TOKEN:
        ...
        print("V73 STOCKNEWS TOKEN_MISSING", flush=True)
        return []
    ...
    if (not force and cached is not None and
        ...
        print(f"V73 STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)
    ...
    try:
        ...
        print(f"V73 STOCKNEWS PROBE HTTP={probe.status_code} | BODY={probe_body or '[empty]'}", flush=True)
        ...
        if main.status_code == 200:
            ...
            if not isinstance(raw_items, list):
                raw_items = []
            print(f"V73 STOCKNEWS MAIN HTTP=200 | RECEIVED={len(raw_items)}", flush=True)
        else:
            print(f"V73 STOCKNEWS MAIN HTTP={main.status_code} | BODY={main_body or '[empty]'} | RETRY=CHUNKS", flush=True)
            chunk_size = 5
            seen_ids = set()
            for i in range(0, len(STOCK_NEWS_COMPANY_SYMBOLS), chunk_size):
                ...
                print(f"V73 STOCKNEWS CHUNK {','.join(chunk)} HTTP={cr.status_code}", flush=True)
        ...
        print(f"V73 STOCKNEWS SUCCESS | RECEIVED={len(raw_items)} | NORMALIZED={len(items)} | SYMBOLS={sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        for idx, it in enumerate(items, 1):
            print(
                f"V73 STOCKNEWS ITEM {idx}/{len(items)} | TICKERS={it.get('stocknews_tickers')} | "
        ...
        print(f"V73 STOCKNEWS ERROR: {safe} — using fallback feeds", flush=True)
        return list(cached or [])
...
def _score_manual_context_news(symbol, item):
    ...
    if not _headline_mentions_symbol(symbol, headline) and not (item.get("trusted_fast_news") and symbol in _symbols_mentioned_in_news(headline, summary)):
        return None
    # Manual context still requires an established source. Unknown/weak aggregators do not pass.
...
def _v73_manual_reject_reason(symbol, item):
...
def _v73_final_gate_reject_reason(item):
...
def scan_manual_stock_news():
    ...
    try:
        for raw in get_stocknews_primary_news():
            ...
            if not syms:
                print(
                    f"V73 STOCKNEWS REJECT | SYMBOL=? | STAGE=SYMBOL_RESOLUTION | REASON=NO_TRACKED_SYMBOL | "
            ...
            for sym in syms:
                ...
                if not scored:
                    print(
                        f"V73 STOCKNEWS REJECT | SYMBOL={sym} | STAGE=MANUAL_SCORE | "
                        f"REASON={_v73_manual_reject_reason(sym, raw)} | SOURCE={raw.get('source') or '?'} | "
    ...
    for raw in (items or []):
        ...
        if not x:
            if raw.get("stocknews_primary") or str(raw.get("news_provider") or "").upper() == "STOCKNEWS":
                print(
                    f"V73 STOCKNEWS REJECT | SYMBOL={raw.get('symbol') or '?'} | STAGE=FINAL_GATE | "
                    f"REASON={_v73_final_gate_reject_reason(raw)} | SOURCE={raw.get('source') or '?'} | "
