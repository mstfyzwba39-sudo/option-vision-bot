...
def format_exact_contract_evaluation(contract):
    spread_text = (
        ...
        if contract["spread_pct"] < 900
        else "غير متوفر"
    )

    if contract["score"] >= 85:
        verdict = "🟢 عقد قوي"
    elif contract["score"] >= 70:
...
def build_opportunity_evaluation(symbol):
    trend = get_stock_trend(symbol)
    price = float(trend.get("price", 0) or 0)
    bias = str(trend.get("bias", "")).upper()
    change = float(trend.get("change_pct", trend.get("change", 0)) or 0)

    direction = "CALL 🟢" if bias == "CALL" else "PUT 🔴" if bias == "PUT" else "محايد 🟡"
    trend_points = 45 if bias in ("CALL", "PUT") else 22
    momentum_abs = abs(change)
    momentum_points = min(momentum_abs / 4.0, 1.0) * 25
    participation_points = 0

    try:
        chain = get_option_chain(symbol)
        contracts = get_top_contracts(chain, trend)
        same_side = [c for c in contracts if str(c.get("side", "")).upper() == bias]
        if same_side:
            best = max(float(c.get("score", 0) or 0) for c in same_side)
            participation_points = min(best / 100.0, 1.0) * 20
    except Exception as exc:
        print("OPPORTUNITY CONFIRMATION ERROR:", exc)

    timing_points = 5 if is_us_market_open() else 2
    score = int(round(min(trend_points + momentum_points + participation_points + timing_points, 100)))

    if score >= 80:
        verdict = "🟢 فرصة قوية — ابحث عن تأكيد دخول"
    elif score >= 65:
        verdict = "🟡 فرصة جيدة — تحتاج تأكيد"
    elif score >= 50:
        verdict = "🟠 احتمال قائم — الأفضل الانتظار"
    else:
        verdict = "🔴 الفرصة ضعيفة حاليًا"

    momentum = "قوي" if momentum_abs >= 3 else "متوسط" if momentum_abs >= 1 else "هادئ"
    ...
    return (
        ...
        f"💵 السعر: ${price:.2f}\\n"
        f"🧭 الاتجاه: {direction}\\n"
        f"⚡ الزخم: {momentum} ({change:+.2f}%)\\n"
        f"📊 تأكيد المشاركة: {'متوفر' if participation_points > 0 else 'غير مكتمل'}\\n"
        "━━━━━━━━━━━━━━\\n"
        f"⭐ قوة الفرصة: {score}/100\\n"
        f"{verdict}\\n\\n"
        "ℹ️ هذا تقييم للفرصة على السهم، وليس تقييمًا لعقد محدد."
    )
