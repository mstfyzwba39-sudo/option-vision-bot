...
async def buttons(
    ...
    if query.data == "top10":
        ...
        try:
            ...
            for item in results:
                # Only yellow/NEAR opportunities need follow-up.
                # Green/INSIDE is already suitable for entry, and WAIT stays out
                # of the confirmation watcher until it becomes near.
                if item.get("entry_zone", {}).get("status") != "NEAR":
                    continue
        ...
        try:
            ...
            if qualified:
                ...
                if signature != LAST_AUTO_TOP_SIGNATURE:
                    msg = (
                        "🚨 تحديث تلقائي — أهم الفرص الآن\n"
                        "🟡 الأصفر = قريب فقط، لا دخول حتى يصل تأكيد الدخول 🟢\n\n"
                    ...
                    for user_id in ALLOWED_USERS:
                        try:
                            ...
                            # IMPORTANT: only yellow/near automatic opportunities are watched.
                            # Green/inside opportunities are already suitable for entry.
                            # Yellow is added to the same 1H confirmation watcher used by manual scans.
                            # The watcher will later send either:
                            #   🟢 تحقق تأكيد الدخول
                            #   🔴 تم استبعاد الفرصة
                            for item in qualified:
                                # Watch yellow/NEAR opportunities only. Green/INSIDE
                                # is already an entry candidate and must not be shown
                                # again as pending confirmation.
                                if item.get("entry_zone", {}).get("status") != "NEAR":
                                    continue
