...
async def monitor_important_news(
    ...
    # HARD DISABLED: news is manual-only from the menu button.
    print("AUTO NEWS HARD DISABLED - MANUAL ONLY")
    return
...
async def post_init(application):
    print("BOT VERSION: MANUAL_NEWS_V2")
    asyncio.create_task(
        monitor_pending(application)
    )

    if FINNHUB_TOKEN:
        # Automatic news push disabled. News remains available from the menu button.
        asyncio.create_task(
        ...
        print(
            "EARNINGS MONITOR STARTED - AUTO NEWS DISABLED"
        )
    else:
        print(
