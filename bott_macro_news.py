...
# كلمات نستخدمها للتأكد أن عنوان الخبر يخص الرمز نفسه فعلاً.
NEWS_SYMBOL_KEYWORDS = {
    "SPY": ["spy", "s&p 500", "s&p500", "spdr s&p 500"],
    "QQQ": ["qqq", "nasdaq 100", "nasdaq-100", "invesco qqq"],
    "IWM": ["iwm", "russell 2000", "ishares russell 2000"],
    "IBIT": ["ibit", "bitcoin", "blackrock bitcoin"],
    "TSLA": ["tsla", "tesla"],
    "NVDA": ["nvda", "nvidia"],
    ...
    "ORCL": ["oracle", "orcl"],
    "OPEN": ["opendoor", "open"],
    "MSTR": ["microstrategy", "strategy inc", "mstr"],
    "COIN": ["coinbase", "coin"],
    "SPCX": ["spacex", "space x", "spcx"],
    "SKHY": ["sk hynix", "skhy"],
}

# أخبار الرأي/المقالات العامة لا نعتبرها "خبرًا مهمًا" وحدها.
OPINION_PHRASES = [
    "technical analysis",
    "strong buy",
    "buying opportunity",
...
# قواعد الأهمية. 9 و10 مخصصة للأحداث التي يمكن أن تحرك السهم فعلاً.
IMPORTANT_NEWS_RULES = [
    (
        10,
        [
            "bankruptcy", "chapter 11", "sec charges", "criminal charges",
            "merger", "buyout", "takeover", "acquisition agreement",
            "to acquire", "will acquire", "fda approval", "fda rejection",
            "trading halt", "halted trading"
        ],
        "حدث جوهري جدًا"
    ),
    (
        9,
        [
            "earnings results", "reports earnings", "quarterly results",
            "raises guidance", "raised guidance", "cuts guidance",
            "cut guidance", "lowers guidance", "profit warning",
            "beats estimates", "misses estimates", "revenue beat",
            "revenue miss", "ceo resigns", "ceo steps down",
            "sec investigation", "doj investigation", "subpoena",
            "antitrust lawsuit", "fraud charges"
        ],
        "نتائج أو تطور جوهري"
    ),
    (
        8,
        [
            "contract win", "wins contract", "awarded contract",
            "strategic partnership", "major partnership",
            "regulatory approval", "share buyback", "stock buyback",
            "repurchase program", "stock split", "share offering",
            "public offering", "capital raise", "layoffs", "job cuts"
        ],
        "خبر مهم قد يؤثر على حركة السهم"
    ),
    (
        7,
        [
            "upgrade", "upgraded", "downgrade", "downgraded",
            "price target", "deliveries", "production",
            "export restriction", "export restrictions",
            "tariff", "antitrust", "regulatory"
        ],
...
POSITIVE_WORDS = [
    ...
    "record sales", "contract win", "wins contract", "awarded contract",
    "buyback", "repurchase", "strong demand", "surge", "surges",
    "growth", "to acquire", "will acquire", "partnership"
]

NEGATIVE_WORDS = [
    "misses estimates", "missed estimates", "cuts guidance", "cut guidance",
    "lowers guidance", "downgrade", "downgraded", "investigation",
    "subpoena", "lawsuit", "fraud", "bankruptcy", "chapter 11",
    "layoffs", "job cuts", "offering", "share sale", "capital raise",
    "weak demand", "decline", "declines", "falls", "drops", "plunge",
    "plunges", "bearish", "rejected", "rejection", "sell-off",
    "profit warning"
]
...
def _phrase_in_text(text, phrase):
    """مطابقة كلمة/عبارة بدون أخطاء مثل recall داخل recalls."""
    import re
...
def _headline_is_relevant(symbol, headline):
    text = (headline or "").lower()
    keywords = NEWS_SYMBOL_KEYWORDS.get(symbol, [symbol.lower()])
    return _has_any_phrase(text, keywords)


def translate_headline_ar(headline):
    """
    ترجمة العنوان للعربية. إذا تعطلت خدمة الترجمة يبقى الإنجليزي
    حتى لا يتعطل زر الأخبار بالكامل.
    """
    headline = str(headline or "").strip()
    if not headline:
        return ""

    cached = NEWS_TRANSLATION_CACHE.get(headline)
    if cached:
        return cached
    ...
    try:
        ...
        if translated:
            NEWS_TRANSLATION_CACHE[headline] = translated
            return translated
    except Exception as e:
        print("NEWS TRANSLATION ERROR:", e)
...
def score_news_item(symbol, item):
    ...
    # أول شرط: عنوان الخبر نفسه يجب أن يخص السهم/الأصل.
    # هذا يمنع أخبارًا مثل ABN AMRO من الظهور تحت SPY.
    if not _headline_is_relevant(symbol, headline):
        return None

    # قصص جانبية/ترفيهية لا تُعامل كخبر تداول حتى لو ظهر في الملخص
    # كلمات مالية عامة. أمثلة: ذكريات، قصص غريبة، ميمز، تصريحات خفيفة.
    noise_phrases = [
        "bizarre", "weird story", "recalls", "remember when",
        "kidnap a seal", "meme", "viral joke", "fun fact",
        "throwback", "shares a funny story", "jokes about"
    ]
    if _has_any_phrase(headline, noise_phrases):
        return None
    ...
    for score, words, label in IMPORTANT_NEWS_RULES:
        if _has_any_phrase(combined, words):
            importance = score
            reason = label
            break

    # مقالات الرأي والتحليل لا ترتفع لمستوى "خبر مهم"
    # إلا إذا احتوت على محفز حقيقي من القواعد أعلاه.
    if _has_any_phrase(headline, OPINION_PHRASES) and importance < 8:
        importance = min(importance, 5)
        reason = "رأي أو تحليل"

    # SeekingAlpha/Yahoo كثيرًا ما تنشر مقالات رأي.
    # نمنع رفعها إلى 9/10 لمجرد كلمات عامة مثل revenue/forecast.
    source_lower = source.lower()
    if source_lower in {"seekingalpha", "seeking alpha", "yahoo"}:
        if importance >= 8 and not _has_any_phrase(
            combined,
            IMPORTANT_NEWS_RULES[0][1]
            + IMPORTANT_NEWS_RULES[1][1]
            + IMPORTANT_NEWS_RULES[2][1]
        ):
    ...
    positive_hits = sum(
        ...
        if _phrase_in_text(combined, word)
    )
    ...
    if _has_any_phrase(
        ...
        [
            ...
            "cuts guidance", "revenue beat", "revenue miss"
        ]
    ):
        ...
        category_ar = "عقد أو شراكة مهمة"
    elif _has_any_phrase(
        combined,
        [
            "merger", "buyout", "takeover",
            "acquisition agreement", "to acquire", "will acquire"
        ]
    ):
        ...
        combined,
        [
            "investigation", "lawsuit", "subpoena",
            "fraud charges", "sec charges", "antitrust"
        ]
    ):
        category_ar = "تطور قانوني أو تنظيمي"
    elif _has_any_phrase(
        combined,
        ["fda approval", "fda rejection", "regulatory approval"]
    ):
        category_ar = "موافقة أو قرار تنظيمي"
    elif _has_any_phrase(
        combined,
        ["layoffs", "job cuts"]
    ):
...
def scan_important_news():
    ...
    for symbol in SCAN_SYMBOLS:
        try:
            ...
            for item in raw_news:
                ...
                # فلتر صارم: لا نعرض إلا الأخبار المؤثرة فعلًا (8/10 فأعلى).
                # أخبار 7/10 تبقى خارج قائمة "أهم الأخبار" حتى لا تدخل
                # القصص الجانبية والمقالات الخفيفة غير القابلة للتداول.
                if scored["importance"] < 8:
                    continue
    ...
    # الترجمة تتم بعد اختيار أفضل النتائج فقط حتى لا نبطئ الفحص.
    for item in results:
        item["headline_ar"] = translate_headline_ar(
            item["headline"]
        )
...
def format_news_item(item):
    ...
    headline_ar = item.get("headline_ar") or headline
...
# =========================================================
# US MACRO CALENDAR - TRADINGVIEW PUBLIC CALENDAR
# =========================================================
...
US_MACRO_KEYWORDS = [
    ...
    "jolts", "consumer confidence", "consumer sentiment",
]

US_MACRO_AR = [
    ("fomc", "قرار/اجتماع الفيدرالي"),
    ("interest rate", "قرار الفائدة الأمريكية"),
    ("federal reserve", "الفيدرالي الأمريكي"),
    ("powell", "تصريحات جيروم باول"),
    ("core cpi", "التضخم الأساسي CPI"),
    ("consumer price index", "مؤشر أسعار المستهلك CPI"),
    ("cpi", "مؤشر أسعار المستهلك CPI"),
    ("core pce", "التضخم الأساسي PCE"),
    ("personal consumption expenditures", "مؤشر نفقات الاستهلاك PCE"),
    ("pce", "مؤشر نفقات الاستهلاك PCE"),
    ("producer price index", "مؤشر أسعار المنتجين PPI"),
    ("ppi", "مؤشر أسعار المنتجين PPI"),
    ("nonfarm payroll", "الوظائف غير الزراعية NFP"),
    ("non-farm payroll", "الوظائف غير الزراعية NFP"),
    ("unemployment rate", "معدل البطالة"),
    ("jobless claims", "طلبات إعانة البطالة"),
    ("initial claims", "طلبات إعانة البطالة"),
    ("gross domestic product", "الناتج المحلي الإجمالي GDP"),
    ("gdp", "الناتج المحلي الإجمالي GDP"),
    ("retail sales", "مبيعات التجزئة"),
    ("ism manufacturing", "ISM الصناعي"),
    ("ism services", "ISM الخدمي"),
    ("jolts", "فرص العمل JOLTS"),
    ("consumer confidence", "ثقة المستهلك"),
    ("consumer sentiment", "معنويات المستهلك"),
]


def _macro_name_ar(title):
    low = str(title or "").lower()
    for key, value in US_MACRO_AR:
        if key in low:
            return value
    return str(title or "").strip()


def _macro_value(value, unit=""):
...
def _parse_tv_datetime(value):
    if not value:
        return None
    text = str(value).strip()
    try:
        # TradingView commonly returns ISO-8601 timestamps ending in Z.
        if text.endswith("Z"):
            text = text[:-1] + "+00:00"
...
def get_us_macro_events(days_back=1, days_forward=7):
    """Fetch high-impact US macro events without adding another API key.

    Uses TradingView's public economic-calendar feed. If the feed changes or
    is unavailable, return an empty list instead of breaking the news button.
    """
    ...
    try:
        ...
        print("US MACRO CALENDAR ERROR:", e)
        return []

    if isinstance(payload, dict):
        raw = payload.get("result") or payload.get("events") or []
    ...
    for item in raw:
        ...
        low = title.lower()
        if not title or not any(key in low for key in US_MACRO_KEYWORDS):
            continue
        ...
        # TradingView importance is commonly 1/2/3. Keep major events only.
        # Some feeds use text labels, so important keywords are still accepted.
        imp_text = str(importance_raw or "").lower()
        if importance and importance < 2:
            continue
        if imp_text in {"low", "1"}:
            continue

        dt = _parse_tv_datetime(
            item.get("date")
...
def format_us_macro_events(events):
    if not events:
        return (
            "🇺🇸 أهم أحداث السوق الأمريكي\n\n"
            "✅ لا توجد أحداث أمريكية عالية التأثير ضمن الفترة الحالية."
        )
    ...
    for item in events:
        dt_riyadh = item["datetime"].astimezone(ZoneInfo("Asia/Riyadh"))
        released = item["datetime"] <= now
        actual = _macro_value(item["actual"], item["unit"])
        forecast = _macro_value(item["forecast"], item["unit"])
        previous = _macro_value(item["previous"], item["unit"])

        status = "✅ صدر" if released else "⏳ قادم"
        fire = "🔥🔥🔥" if item["importance"] >= 3 else "🔥🔥"

        block = (
            f"🇺🇸 {item['title_ar']}\n"
            f"{fire} {status}\n"
            f"🕐 {dt_riyadh.strftime('%d/%m %I:%M %p')} بتوقيت السعودية\n"
        )

        if actual != "—":
            block += f"📊 الفعلي: {actual}\n"
        if forecast != "—":
            block += f"🎯 المتوقع: {forecast}\n"
        if previous != "—":
            block += f"↩️ السابق: {previous}\n"

        block += f"🌐 Original: {item['title']}"
        blocks.append(block)

    return (
        "🇺🇸 أهم أحداث السوق الأمريكي\n"
        "📌 الفيدرالي • التضخم • الوظائف • النمو\n"
        "━━━━━━━━━━━━━━\n\n"
        + "\n\n━━━━━━━━━━━━━━\n\n".join(blocks)
    )
...
async def buttons(
    ...
    if query.data == "top10":
        ...
        await query.message.reply_text(
            "📰 جاري فحص أخبار أسهمنا "
            "وأحداث السوق الأمريكي..."
        )
