...
def get_upcoming_us_events():
    ...
    now = datetime.now(ZoneInfo("Asia/Riyadh"))
    ordinary_end = now + timedelta(
        days=UPCOMING_EVENT_DAYS
    )
    ...
    # Always include the NEXT major event of each type even if it is
    # more than 7 days away (up to the official 60-day fetch horizon).
    for major_name in MAJOR_NAMES:
        candidates = [
            event
            for event in all_events
            if (
                event["name"] == major_name
                and event["dt"] >= now
            )
        ]
        if candidates:
...
def format_upcoming_us_events():
    ...
    if errors:
        result += (
            "\n\n⚠️ ملاحظة: تعذر تحديث أحد المصادر الرسمية؛ "
            "تم عرض المواعيد التي تم التحقق منها فقط."
        )
