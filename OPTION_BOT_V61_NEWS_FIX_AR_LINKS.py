...
def get_us_market_news():
    ...
    if FINNHUB_TOKEN:
        try:
            ...
            print(f"US MARKET FINNHUB ERROR: {type(exc).__name__}: {exc}", flush=True)

    try:
        query = (
            '("Federal Reserve" OR FOMC OR "US jobs" OR "U.S. jobs" OR '
            'CPI OR PPI OR PCE OR "Treasury yields" OR "Wall Street futures" OR '
            '"S&P 500 futures" OR "Nasdaq futures" OR Iran OR "Strait of Hormuz" OR '
            '"oil prices" OR tariffs OR "trade war") when:1d'
        )
        response = requests.get(
            FAST_NEWS_RSS_URL,
            headers={
...
def _arabize_fed_detail(detail):
    ...
    # Keep policymaker names recognizable while Arabicizing titles.
