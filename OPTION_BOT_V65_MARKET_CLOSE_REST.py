...
def get_option_chain(symbol):
    ...
    start_date = today if str(symbol).upper().strip() == "SPX" else (today + timedelta(days=1))

    params = {
        "from": start_date.isoformat(),
        "to": (today + timedelta(days=30)).isoformat(),
        "minAsk": MIN_OPTION_ASK,
...
def _translate_headline_ar_google(headline):
    ...
    try:
        ...
        # V47 finance-language cleanup for natural Arabic display.
...
def format_combined_news():
    # V57: news is independent from the upcoming-events calendar.
    macro_items=scan_us_market_news(); stock_items=(NEWS_CACHE.get("results") or _load_verified_daily_news())
...
def get_top_contracts(data, trend, soft_direction=False):
    ...
    for i in range(count):
        try:
            ...
            # Stocks: 1-30 DTE. SPX/SPXW: allow same-day (0DTE) through 30 DTE.
            # Detect SPX from the option contract itself; `symbol` is not in scope here.
            is_spx_contract = str(option_symbol).upper().strip().startswith("SPX")
            min_dte = 0 if is_spx_contract else 1
            if dte < min_dte or dte > 30:
                continue
            ...
            # SPX contracts priced $1-$5 are naturally farther OTM than stock options.
            # Keep the normal 0.20-0.80 delta quality gate for stocks, but use a
            # dedicated SPX gate so affordable SPX contracts are not eliminated by design.
            delta_abs = abs(delta)
            if is_spx_contract:
                if delta_abs < 0.05 or delta_abs > 0.50:
                    continue
            elif delta_abs < 0.20 or delta_abs > 0.80:
                continue
...
def add_active_contract(
    ...
    """Register any confirmed opportunity in the shared active-contract tracker.

    Used by both paths:
    1) a yellow opportunity that later becomes confirmed, and
    2) an opportunity that is already GREEN when the automatic update first sends it.
    Existing active entries are never overwritten, so the original entry price remains intact.
    """
    option_symbol = str(contract.get("option_symbol", "") or "")
    if not option_symbol:
        return False

    active_key = f"{chat_id}:{option_symbol}"
    if active_key in ACTIVE_CONTRACTS:
        return False

    ask = float(entry_ask or contract.get("ask", 0) or 0)
    if ask < MIN_OPTION_ASK or ask > MAX_OPTION_ASK:
        return False
    ...
    item = {
        ...
        "expiry_date": str(contract.get("expiry_date", "")),
        "option_symbol": option_symbol,
        "entry_ask": ask,
        "highest_ask": ask,
        "entry_message_id": int(entry_message_id or 0),
        ...
        "loss_50_sent": False,
        "reverse_sent": False,
        "high_risk_sent": False,
        "review_sent": False,
    }

    ACTIVE_CONTRACTS[active_key] = item
            ...
            try:
                ...
                if ask_now < MIN_OPTION_ASK or ask_now > MAX_OPTION_ASK:
                    await application.bot.send_message(
                        chat_id=chat_id,
                        text=(
                            "⚠️ تحقق التأكيد الفني لكن سعر العقد خارج النطاق $1–$5\n\n"
                            f"{symbol} "
                            f"{side} "
                            f"{strike:g} | "
                            f"{expiry_date}\n"
                ...
                confirmed_contract = dict(option_now)
                confirmed_contract.update({
                    "side": side,
                    "strike": strike,
                    ...
                    if market_open and now_ny.date() <= expiry_ny:
                        ...
                        if quote and float(quote.get("ask", 0) or 0) > 0:
                            ...
                            if entry > 0:
                                ...
                                # Profit milestones: each level alerts once.
                                for level in (30, 60, 100, 150, 200):
                                    flag = f"profit_{level}_sent"
                                    if pnl_pct >= level and not item.get(flag):
                                        await _send_contract_alert(application, item,
                                            f"💰 ربح العقد وصل {level}٪\n\n"
                                            f"{line}\n💵 السعر الآن ${price:.2f} ({pnl_pct:+.1f}٪)\n🎯 سعر الدخول ${entry:.2f}")
                                        item[flag] = True
                                        hist[flag] = True
                                        hist["status"] = "SUCCESS"
                                        state_changed = True
                                ...
                                if pnl_pct <= -50 and not item.get("loss_50_sent"):
                                    best_pct = float(hist.get("highest_pnl_pct", 0) or 0)
                                    reached = [lvl for lvl in (30, 60, 100, 150, 200) if best_pct >= lvl]
                                    if reached:
                                        best_level = max(reached)
                                        alert_title = f"⚠️ تراجع العقد بعد وصول الربح إلى +{best_level}٪"
                                        extra = f"\n🚀 أعلى ربح فعلي وصل له {best_pct:+.1f}٪"
                                    else:
                                        alert_title = "🔴 خسارة العقد وصلت -50٪"
                                        extra = ""
                                    await _send_contract_alert(application, item,
                                        f"{alert_title}\n\n"
                                        f"{line}\n💵 السعر الآن ${price:.2f} ({pnl_pct:+.1f}٪)\n🎯 سعر الدخول ${entry:.2f}{extra}")
                                    item["loss_50_sent"] = True
                                    hist["loss_50_sent"] = True
                                    if not reached and hist.get("status") != "SUCCESS":
                                        hist["status"] = "LOSS"
                                    state_changed = True
                            ...
                            if not item.get("reverse_sent"):
                                ...
                                if risk_level == "EXIT":
                                    ...
                                    await _send_contract_alert(application, item,
                                        ...
                                        f"💵 سعر الدخول ${entry0:.2f}\n"
                                        f"💵 السعر الآن ${price:.2f} ({current_pnl:+.1f}٪)\n"
                                        f"🚀 أعلى ربح سابق {float(hist.get('highest_pnl_pct', 0) or 0):+.1f}٪\n"
...
async def buttons(
    ...
    if query.data == "top10":
        ...
        try:
            ...
            top_message = await query.message.reply_text(
            ...
            for item in results:
                state0 = str(item.get("decision", {}).get("state", "")).upper()
                contract = item["contract"]
                trend = item["trend"]

                # Every already-confirmed GREEN opportunity enters the same
                # profit/reversal/expiry tracker immediately.
                if state0 == "GREEN":
                    add_active_contract(
                        chat_id,
                        item["symbol"],
                        contract,
                        float(contract.get("ask", 0) or 0),
                        top_message.message_id,
                    )
                    PENDING_WATCHES.pop(
                        watch_key(chat_id, item["symbol"]),
                        None,
                    )
                    continue

                # Yellow opportunities keep waiting for 1H confirmation; once
                # confirmed, monitor_pending moves them into the same tracker.
                if state0 != "YELLOW":
                    continue
...
# =========================================================
# PROGRAM PULSE — HOURLY STATUS WITHOUT USING GREEN CONFIRMATION ICON
# =========================================================
...
async def monitor_program_pulse(application):
    # Wait one hour: opportunity scanning continues every 15 minutes meanwhile.
    await asyncio.sleep(PROGRAM_PULSE_INTERVAL_SECONDS)
    while True:
        try:
            # The hourly pulse belongs to regular U.S. market hours only.
            # After the close, the dedicated rest message is sent instead.
            if not is_us_market_open():
                await asyncio.sleep(PROGRAM_PULSE_INTERVAL_SECONDS)
                continue

            last_ts = float(PROGRAM_PULSE_STATE.get("last_success_ts", 0) or 0)
            if last_ts > 0:
                last_sa = datetime.fromtimestamp(last_ts, ZoneInfo("Asia/Riyadh"))
                last_text = last_sa.strftime("%I:%M %p").lstrip("0")
                qualified_count = int(PROGRAM_PULSE_STATE.get("qualified_count", 0) or 0)
                for chat_id in ALLOWED_USERS:
                    ...
                    text = (
                        ...
                        f"🔎 آخر فحص ناجح: {last_text} 🇸🇦\n"
                        f"🎯 الفرص المؤهلة الآن: {qualified_count}\n"
                        f"👀 العقود تحت المتابعة: {active_count}\n"
                        "⏱️ فحص الفرص مستمر كل 15 دقيقة"
                    )
                    await application.bot.send_message(chat_id=chat_id, text=text)
            else:
                # Do not claim the program completed a market scan if none has succeeded yet.
                print("PROGRAM PULSE: waiting for first successful opportunity scan", flush=True)
        except Exception as exc:
...
async def monitor_market_close_rest(application):
    """Send one friendly end-of-day message when the regular U.S. market closes."""
    ...
    while True:
        try:
            ...
            # Send only on the open -> closed transition, once per New York date.
            if was_open and not market_open and ny_now.weekday() < 5:
                today_key = ny_now.date().isoformat()
                if last_sent_ny_date != today_key:
                    text = (
                        "🌙 السوق أغلق لليوم\n"
                        "🤖 البرنامج خلّص متابعة السوق والفرص\n"
                        "📊 العقود المفتوحة محفوظة وبيكمل متابعتها مع افتتاح السوق القادم\n"
                        "😴 نرتاح الحين.. ونرجع لكم بكرة بإذن الله"
                    )
                    for chat_id in ALLOWED_USERS:
                        await application.bot.send_message(chat_id=chat_id, text=text)
                    last_sent_ny_date = today_key
                    print("MARKET CLOSE REST MESSAGE SENT", flush=True)

            was_open = market_open
        except Exception as exc:
            print("MARKET CLOSE REST ERROR:", repr(exc), flush=True)

        # A one-minute check catches the close promptly without adding load.
...
async def monitor_auto_top_opportunities(application):
    ...
    The first scan of each process starts with a fresh *session signature* so a
    deploy/restart can surface currently-qualified opportunities that were not
    previously auto-sent. Persistent AUTO_OPPORTUNITY_HISTORY is still honored,
    therefore any exact contract already auto-sent remains blocked until +50%.
    Active-contract tracking is stored separately and is never reset here.

    Later scans run every 15 minutes while the U.S. market is open, and a small
    score/price change alone never causes a repeat message.
    """
    global LAST_AUTO_TOP_SIGNATURE

    first_scan_this_process = True
    ...
    while True:
        try:
            ...
            # A completed scan updates the hourly pulse. The pulse never uses the
            # green confirmation icon; that remains exclusive to confirmed opportunities.
            ...
            previous_state = {}
            # On a fresh process, do not let yesterday's/previous-deploy signature
            # silence today's first scan. We intentionally keep
            # AUTO_OPPORTUNITY_HISTORY untouched, so an exact contract already
            # auto-sent and still below +50% remains suppressed.
            if (not first_scan_this_process) and isinstance(LAST_AUTO_TOP_SIGNATURE, (list, tuple)):
            ...
            if triggered:
                ...
                for user_id in ALLOWED_USERS:
                    try:
                        ...
                        # Every confirmed opportunity enters the SAME active tracker,
                        # even when it is GREEN on its very first automatic appearance.
                        # Yellow opportunities continue through the 1H pending watcher.
                        for item in triggered:
                            state0 = str(item.get("decision", {}).get("state", "")).upper()
                            contract = item["contract"]
                            trend = item["trend"]
                            ...
                            if state0 == "GREEN":
                                ...
                                # If this was previously yellow, stop its pending watcher
                                # so it cannot create a duplicate entry/confirmation later.
                                PENDING_WATCHES.pop(
                                    watch_key(user_id, item["symbol"]),
                                    None,
                                )
                                if added:
                                    print(
                                        "AUTO GREEN ADDED TO ACTIVE TRACKER:",
                                        user_id,
                                        item["symbol"],
                                        contract.get("option_symbol"),
                                        flush=True,
            ...
            first_scan_this_process = False
            _save_auto_sent_state()

        except Exception as exc:
            PROGRAM_PULSE_STATE["last_error"] = repr(exc)
...
async def post_init(application):
    ...
    # Lightweight hourly proof-of-life; opportunity scanning itself stays every 15 minutes.
    asyncio.create_task(monitor_program_pulse(application))
    print("HOURLY PROGRAM PULSE STARTED")

    # At the regular U.S. market close, send one friendly end-of-day rest message.
    asyncio.create_task(monitor_market_close_rest(application))
    print("MARKET CLOSE REST WATCHER STARTED")
