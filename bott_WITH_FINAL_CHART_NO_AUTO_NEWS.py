...
async def post_init(application):
    # Keep automatic opportunity monitoring.
    asyncio.create_task(
        monitor_pending(application)
    )

    # IMPORTANT:
    # Do NOT send stock/news alerts automatically.
    # News should be sent only when the user presses the News button.
    if FINNHUB_TOKEN:
        asyncio.create_task(
        ...
        print(
            "AUTO NEWS DISABLED — EARNINGS REMINDERS ENABLED"
        )
    else:
        print(
