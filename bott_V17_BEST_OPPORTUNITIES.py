...
def scan_top10():
    ...
    for symbol in SCAN_SYMBOLS:
        try:
            ...
            contract = matching[0]
            decision = effective_decision(contract, trend)

            # During open market, only a live-confirmed stock opportunity
            # can be presented as a true "best opportunity".
            if metrics["market_open"]:
                if metrics["live_status"] != "CONFIRMED" or metrics["score"] < 82:
                    continue
                opportunity_label = "🟢 فرصة مؤكدة"
            else:
                # Closed market: candidates may be shown, but explicitly as watchlist.
                if metrics["score"] < 68:
                    continue
...
def format_top10(results):
    ...
    if market_open:
        title = f"🏆 أفضل {len(results)} فرص مؤكدة لليوم"
