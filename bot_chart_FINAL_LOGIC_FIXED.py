...
def make_chart(symbol, resolution):
    ...
    if demand_zone:
        ...
        ax.text(
            ...
            color="#202020", fontsize=12.5, fontweight="bold", va="center", ha="left", zorder=2,
    ...
    # Expected path: draw ONLY when the setup already has a directional bias.
    # A NEUTRAL setup means WAIT: no forecast line is drawn, because connecting
    # the current price to an unconfirmed trigger creates a misleading prediction.
    if chart_targets and direction in ("BULLISH", "BEARISH"):
        path_color = green if direction == "BULLISH" else red
        demand_zone = setup.get("demand_zone")
        trigger = setup["trigger"]
        atr_local = max(calculate_atr(highs, lows, closes, 14), last_price * 0.002)

        # Forecast divider: everything to its right is future/conditional only.
        ax.vlines(
            future_start - 0.55, min(lows), max(highs),
            color="#D8D3C8", linewidth=1.0, linestyle=(0, (2, 5)), alpha=0.65, zorder=0,
        )

        confirmed = (last_price >= trigger) if direction == "BULLISH" else (last_price <= trigger)
        points_y = [last_price]

        if direction == "BULLISH":
            if not confirmed:
                # One modest future retest only, then confirmation.
                retest = last_price - atr_local * 0.35
                if demand_zone:
                    demand_mid = (demand_zone["lower"] + demand_zone["upper"]) / 2
                    if 0 < last_price - demand_mid <= atr_local * 1.25:
                        retest = max(demand_mid, last_price - atr_local * 0.60)
                if retest < last_price - atr_local * 0.10:
            ...
            if not confirmed:
                retest = min(last_price + atr_local * 0.35, setup["invalidation"] - atr_local * 0.10)
                if retest > last_price + atr_local * 0.10:
                    points_y.append(retest)
                points_y.append(trigger)
        ...
        if future_targets:
            ...
            if len(future_targets) > 1:
                # One shallow pause after T1, then continue to the remaining targets.
                pause_anchor = trigger if not confirmed else last_price
                pause = future_targets[0] + (pause_anchor - future_targets[0]) * 0.18
        ...
        if len(points_y) >= 2:
            # All forecast x values are strictly to the RIGHT of the last candle.
            path_x0 = x_last + 1.35
            path_x1 = future_end - 0.80
            step = (path_x1 - path_x0) / max(1, len(points_y) - 1)
            points_x = [path_x0 + i * step for i in range(len(points_y))]

            ax.plot(
                points_x, points_y,
                color=path_color, linewidth=2.5, alpha=0.92,
                solid_capstyle="round", solid_joinstyle="round", zorder=5,
            )
            ax.scatter(points_x, points_y, s=18, color=path_color, zorder=6)
            ax.annotate(
                "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
                arrowprops={"arrowstyle":"-|>","color":path_color,"lw":2.6,"alpha":0.96}, zorder=6,
            )

            # Place the label in empty future space, not on levels or candles.
            label_x = path_x0 + 0.20
            label_y = max(points_y[0], points_y[1]) + atr_local * 0.55 if direction == "BULLISH" else min(points_y[0], points_y[1]) - atr_local * 0.55
            ax.text(
                label_x, label_y, "EXPECTED PATH",
                color="#202020", fontsize=12.5, fontweight="bold",
                va="bottom" if direction == "BULLISH" else "top", zorder=7,
                bbox={"boxstyle":"round,pad=0.16","facecolor":"#FFFDF8","edgecolor":"#D7BE80","linewidth":0.8,"alpha":0.96},
            )

    elif direction == "NEUTRAL":
        # Neutral = no directional forecast. Keep the chart honest: only show the
        # conditional confirmation level and targets. The path appears only after
        # a real bullish/bearish setup is detected on a later refresh.
        ax.text(
            future_start + 0.15, setup["trigger"] + max(calculate_atr(highs, lows, closes, 14), last_price * 0.002) * 0.45,
            "WAIT FOR CONFIRMATION",
            color="#5F5A50", fontsize=12.5, fontweight="bold", ha="left", va="bottom",
            bbox={"boxstyle":"round,pad=0.16","facecolor":"#FFFDF8","edgecolor":"#D7BE80","linewidth":0.8,"alpha":0.96},
            zorder=7,
        )
