...
def build_opportunity_evaluation(symbol):
    ...
    # Use the actual keys produced by the stock-trend engine.
    price = float(
        trend.get("last_close",
        trend.get("price", 0)) or 0
    )
    bias = str(
    ...
    change = float(
        ...
        trend.get("change", 0)) or 0
    )
    momentum_score = float(
        trend.get("momentum_score", 0) or 0
    )

    # Never score a stock if core market data is missing.
    if price <= 0:
        raise ValueError(
            "تعذر الحصول على سعر السهم بشكل صحيح، "
            "لذلك لم يتم تقييم الفرصة."
        )
    ...
    # Momentum is based primarily on the bot's existing momentum engine.
    momentum_points = min(
    ...
    # Options participation is only a confirmation of the underlying opportunity.
    ...
    return (
        ...
        f"{verdict}\n"
        "━━━━━━━━━━━━━━\n\n"
        "ℹ️ هذا تقييم للفرصة على السهم، "
        "وليس تقييمًا لعقد أوبشن محدد."
    )
