...
def make_chart(symbol, resolution):
    ...
    scenario_invalidated = (
        (direction == "BULLISH" and last_price <= float(setup["invalidation"]))
        or
        (direction == "BEARISH" and last_price >= float(setup["invalidation"]))
    )
...
def format_combined_news():
    ...
    # Keep auto-sent material stories visible in the manual "أهم الأخبار" list
    # for the rest of the same Saudi day, even if a live feed later omits them.
    remembered_today = []
    for old in SENT_NEWS_EVENTS:
        try:
            nts = float(old.get("news_timestamp") or old.get("timestamp") or 0)
            if datetime.fromtimestamp(nts, ZoneInfo("Asia/Riyadh")).date() != today_ksa:
                continue
            if not old.get("headline") or not old.get("symbol"):
                continue
            importance = int(old.get("importance") or 0)
            # Older state records from pre-V27 may not have full display fields.
            # Only merge records that can be rendered reliably.
            if importance < 7:
                continue
            remembered_today.append({
                "id": f"remembered:{old.get('symbol')}:{nts}:{old.get('headline')}",
                "symbol": str(old.get("symbol") or "").upper(),
                "headline": str(old.get("headline") or ""),
                "summary": str(old.get("summary") or ""),
                "source": str(old.get("source") or ""),
                "url": str(old.get("url") or ""),
                "timestamp": nts,
                "importance": importance,
                "reason_ar": str(old.get("reason_ar") or old.get("category") or "خبر مهم للسهم"),
                "category_ar": str(old.get("category") or old.get("reason_ar") or "خبر مهم للسهم"),
                "sentiment": str(old.get("sentiment") or ""),
                "sentiment_ar": str(old.get("sentiment_ar") or (
                    "🟢 إيجابي" if str(old.get("sentiment") or "").upper() == "POSITIVE"
                    else "🔴 سلبي" if str(old.get("sentiment") or "").upper() == "NEGATIVE"
                    else "⚪ محايد"
                )),
            })
        except Exception:
    ...
    if remembered_today:
        stock_items.extend(remembered_today)
        stock_items = _dedupe_news_results(stock_items)
