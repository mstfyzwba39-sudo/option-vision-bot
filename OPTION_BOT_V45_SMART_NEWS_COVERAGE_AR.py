...
POSITIVE_WORDS = [
    ...
    "raises guidance", "raised guidance", "raise guidance", "raised its guidance",
    "lifts guidance", "lifted guidance", "boosts guidance", "boosted guidance",
    "raises outlook", "raised outlook", "lifts outlook", "lifted outlook",
    "boosts outlook", "boosted outlook", "raises forecast", "raised forecast",
    "lifts forecast", "lifted forecast", "boosts forecast", "boosted forecast",
    "forecasts above", "forecast above", "outlook above", "guidance above",
    "tops estimates", "top estimates", "tops expectations", "exceeds estimates",
    "exceeded estimates", "exceeds expectations",
...
NEGATIVE_WORDS = [
    ...
    "reduces guidance", "reduced guidance", "cuts outlook", "cut outlook",
    "lowers outlook", "lowered outlook", "cuts forecast", "cut forecast",
    "lowers forecast", "lowered forecast", "forecast below", "outlook below",
    "guidance below", "falls short of estimates", "below estimates", "downgraded",
...
def _material_query_for_symbol(symbol):
    ...
    material = (
        ...
        'lawsuit OR acquisition OR merger OR earnings OR results OR guidance OR outlook OR forecast OR '
        '"raises guidance" OR "lifts guidance" OR "raises outlook" OR "lifts outlook" OR '
        '"raises forecast" OR "lifts forecast" OR "cuts guidance" OR "cuts outlook" OR '
        '"revenue forecast" OR "revenue outlook" OR "profit warning" OR '
        '"beats estimates" OR "beats expectations" OR "misses estimates" OR "misses expectations" OR '
...
def score_news_item(symbol, item):
    ...
    result_markers = [
        ...
        "beats expectations", "misses expectations", "raises guidance", "raised guidance",
        "lifts guidance", "lifted guidance", "boosts guidance", "raises outlook",
        "raised outlook", "lifts outlook", "raises forecast", "raised forecast",
        "lifts forecast", "reaffirms guidance", "lowers guidance", "cuts guidance",
        "cuts outlook", "lowers outlook", "cuts forecast", "lowers forecast",
        "revenue forecast", "revenue outlook", "quarterly results", "record revenue"
    ]
...
def _news_event_signature(item):
    ...
    groups = [
        ...
        ("earnings_guidance", ["earnings", "quarterly results", "q1", "q2", "q3", "q4", "guidance", "outlook", "forecast", "eps", "revenue"]),
...
def _translate_headline_ar_google(headline):
    ...
    # V45 newsroom wording for earnings/guidance headlines.
    # Avoid literal translations such as "الإرشادات" or translating Salesforce as "قوة المبيعات".
    company_ar = {
    ...
    named_company = next((ar for en, ar in company_ar.items() if en in _headline_l), None)
    if named_company and any(x in _headline_l for x in ["earnings", "guidance", "outlook", "forecast", "quarterly results"]):
        positive_fin = any(x in _headline_l for x in [
            "beats", "beat estimates", "tops estimates", "exceeds", "raises guidance",
            "raised guidance", "lifts guidance", "raises outlook", "lifts outlook",
            "raises forecast", "lifts forecast", "boosts guidance", "boosts outlook"
        ])
        negative_fin = any(x in _headline_l for x in [
            "misses", "falls short", "cuts guidance", "lowers guidance",
            "cuts outlook", "lowers outlook", "cuts forecast", "lowers forecast"
        ])
        if positive_fin and ("guidance" in _headline_l or "outlook" in _headline_l or "forecast" in _headline_l):
            polished = f"{named_company} تتجاوز التوقعات وترفع توقعاتها المالية"
            _HEADLINE_AR_CACHE[text] = polished
            return polished
        if negative_fin and ("guidance" in _headline_l or "outlook" in _headline_l or "forecast" in _headline_l):
            polished = f"{named_company} تسجل نتائج أضعف وتخفض توقعاتها المالية"
            _HEADLINE_AR_CACHE[text] = polished
            return polished
    ...
    while True:
        try:
            ...
            if not baseline_ready:
                ...
                baseline_ready = True
                print(
                    f"NEWS MONITOR V45 READY — BASELINED {baselined} CURRENT ITEMS; "
