...
STOCK_NEWS_API_ITEMS = min(100, max(10, int(os.environ.get("STOCK_NEWS_API_ITEMS", "100"))))
...
def get_stocknews_primary_news(force=False):
    ...
    if not STOCK_NEWS_API_TOKEN:
        ...
        print("V71 STOCKNEWS TOKEN_MISSING", flush=True)
        return []
    ...
    if (not force and cached is not None and
        ...
        print(f"V71 STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)

    # Stage 1: official minimal request shape. This isolates auth/account issues
    # from filters, ticker batching and optional fields.
    probe_params = {"tickers": "AMD", "items": 5, "page": 1, "token": STOCK_NEWS_API_TOKEN}
    try:
        probe, probe_body = _stocknews_request(probe_params)
        print(f"V71 STOCKNEWS PROBE HTTP={probe.status_code} | BODY={probe_body or '[empty]'}", flush=True)
        ...
        if main.status_code == 200:
            ...
            if not isinstance(raw_items, list):
                raw_items = []
            print(f"V71 STOCKNEWS MAIN HTTP=200 | RECEIVED={len(raw_items)}", flush=True)
        else:
            print(f"V71 STOCKNEWS MAIN HTTP={main.status_code} | BODY={main_body or '[empty]'} | RETRY=CHUNKS", flush=True)
            chunk_size = 5
            seen_ids = set()
            for i in range(0, len(STOCK_NEWS_COMPANY_SYMBOLS), chunk_size):
                chunk = STOCK_NEWS_COMPANY_SYMBOLS[i:i+chunk_size]
                cp = {"tickers": ",".join(chunk), "items": 25, "page": 1, "token": STOCK_NEWS_API_TOKEN}
                cr, cb = _stocknews_request(cp)
                print(f"V71 STOCKNEWS CHUNK {','.join(chunk)} HTTP={cr.status_code}", flush=True)
        ...
        print(f"V71 STOCKNEWS SUCCESS | RECEIVED={len(raw_items)} | NORMALIZED={len(items)} | SYMBOLS={sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        return list(items)
    except Exception as exc:
        ...
        print(f"V71 STOCKNEWS ERROR: {safe} — using fallback feeds", flush=True)
        return list(cached or [])
...
def format_combined_news():
    ...
    print(
        f"V71 DIAG STOCKNEWS HTTP={STOCK_NEWS_DIAG.get('http')} | "
