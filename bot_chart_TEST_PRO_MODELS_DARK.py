...
def make_chart(symbol, resolution):
    ...
    candle_width = 0.66
    bullish_candle = "#FFFFFF"
    bearish_candle = "#202020"
    wick_color = "#202020"
    gold = "#B8871E"
    gold_soft = "#D7BE80"
    green = "#2F7D4A"
    ...
    if demand_zone:
        ...
        ax.text(
            ...
            color="#1F1F1F", fontsize=14.5, fontweight="bold", va="bottom", zorder=2,
        )
    ...
    if chart_targets:
        ...
        if len(points_y) >= 2:
            ...
            ax.plot(
                points_x, points_y, color=path_color, linewidth=3.6, alpha=0.96,
                solid_capstyle="round", solid_joinstyle="round", zorder=5,
            )
            ax.scatter(points_x, points_y, s=28, color=path_color, zorder=6)
            ax.annotate(
                "", xy=(points_x[-1], points_y[-1]), xytext=(points_x[-2], points_y[-2]),
                arrowprops={"arrowstyle": "-|>", "color": path_color, "lw": 3.6, "alpha": 0.98},
                zorder=6,
            )
            label = "EXPECTED PATH" if direction != "NEUTRAL" else "PATH AFTER CONFIRM"
            label_i = 1 if len(points_y) > 2 else 0
            ax.text(
                points_x[label_i], points_y[label_i], label, color="#202020", fontsize=14.5,
