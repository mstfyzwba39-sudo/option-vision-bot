...
def get_entry_zone_status(symbol, bias):
    ...
    Primary timeframe: 1 hour. If hourly data is unavailable, use daily data
    with the same supply/demand calculation instead of returning a blank zone.
    """
    last_exc = None
    ...
    for resolution in ("60", "D"):
        try:
            ...
            # LTR isolate the whole price range so Telegram does not reverse it.
            zone_text = (
                "\u2066"
                f"${zone_low:.2f}–${zone_high:.2f}"
                "\u2069"
