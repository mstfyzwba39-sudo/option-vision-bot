...
def score_macro_news(item):
    h=str(item.get("headline") or "").strip(); s=str(item.get("summary") or "").strip(); ts=int(item.get("datetime") or 0)
    if not h or not ts or time.time()-ts > MACRO_MAX_AGE_HOURS*3600 or time.time()-ts < -300: return None
    c=_normalize_news_text(f" {h} {s} ")
    importance=0; category=""
    for score,words,label in MACRO_RULES:
        if any(w in c for w in words): importance,category=score,label; break
    if not importance or any(x in c for x in ANALYSIS_ONLY_WORDS): return None
    return {"headline":h,"source":str(item.get("source") or "").strip(),"url":str(item.get("url") or "").strip(),
            "timestamp":ts,"importance":importance,"category_ar":category}
...
def format_combined_news():
    ...
    a=("🇺🇸 أهم أخبار السوق الأمريكي\n━━━━━━━━━━━━━━\n\n"+"\n\n━━━━━━━━━━━━━━\n\n".join(format_macro_item(x) for x in macro_items)
       if macro_items else f"🇺🇸 أخبار السوق الأمريكي\n\n✅ لا توجد أخبار اقتصادية أمريكية جوهرية حديثة خلال آخر {MACRO_MAX_AGE_HOURS} ساعة.")
