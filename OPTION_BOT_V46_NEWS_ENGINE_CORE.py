...
def _material_query_for_symbol(symbol):
    ...
    material = (
        ...
        '"raises guidance" OR "raised guidance" OR "lifts guidance" OR "lifted guidance" OR '
        '"raises outlook" OR "raised outlook" OR "lifts outlook" OR "lifted outlook" OR '
        '"raises forecast" OR "raised forecast" OR "revenue forecast" OR "revenue outlook" OR '
        '"beats estimates" OR "beats expectations" OR "misses estimates" OR "misses expectations" OR '
        '"profit warning" OR "CEO resigns" OR "CEO steps down" OR "FDA approval" OR "FDA rejects" OR '
...
_MATERIAL_NEWS_CACHE = {"time": 0.0, "items": []}
MATERIAL_NEWS_CACHE_SECONDS = 600

def get_material_backup_news():
    """Per-company high-impact Google News search, cached to keep the Telegram button fast."""
    now = time.time()
    if _MATERIAL_NEWS_CACHE["items"] and now - _MATERIAL_NEWS_CACHE["time"] < MATERIAL_NEWS_CACHE_SECONDS:
        return list(_MATERIAL_NEWS_CACHE["items"])
    items = []
    headers = {
    ...
    _MATERIAL_NEWS_CACHE["time"] = now
    _MATERIAL_NEWS_CACHE["items"] = list(items)
    return items
...
def _same_news_event(a, b):
    ...
    if sig_a and sig_b and sig_a == sig_b:
        # Earnings/guidance stories are often split into "EPS beat", "raises outlook",
        # "shares jump" and partnership follow-ups. Treat them as one company event
        # inside the same 24-hour reporting window.
        return True
...
def _translate_headline_ar_google(headline):
    ...
    try:
        ...
        # Financial-news Arabic cleanup. Preserve brands and replace literal finance jargon.
        translated = re.sub(r"قوة المبيعات", "Salesforce", translated, flags=re.I)
        translated = re.sub(r"إرشادات", "توقعات", translated)
        translated = re.sub(r"التوجيه(?:ات)?", "التوقعات", translated)
        translated = re.sub(r"يسحق الأرباح", "يتجاوز توقعات الأرباح", translated)
        translated = re.sub(r"تسحق الأرباح", "تتجاوز توقعات الأرباح", translated)
        if translated:
    ...
    while True:
        try:
            ...
            if not baseline_ready:
                ...
                baseline_ready = True
                print(
                    f"NEWS MONITOR V46 READY — BASELINED {baselined} CURRENT ITEMS; "
