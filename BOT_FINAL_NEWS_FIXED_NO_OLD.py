...
def score_news_item(symbol, item):
    ...
    finnhub_direct_match = _direct_match(symbol, item, combined)
    if not headline_subject_match and not finnhub_direct_match:
        return None
...
async def monitor_major_us_events(application):
    """Send one reminder about 12 hours before each major U.S. market event."""
    ...
    while True:
        try:
            ...
            for event in events:
                ...
                if seconds < 0 or seconds > EVENT_REMINDER_HOURS * 3600:
                    continue
                ...
                hours_left = max(0, round(seconds / 3600, 1))
                text_msg = (
                    "🚨 تذكير بحدث أمريكي مهم\n\n"
                    f"🇺🇸 {event['name']}\n"
                    f"⏰ {event['dt'].strftime('%I:%M %p')} بتوقيت السعودية\n"
                    f"⏳ متبقي تقريبًا {hours_left} ساعة\n"
...
async def post_init(application):
    ...
    # Major U.S. scheduled events: automatic reminder before the event.
