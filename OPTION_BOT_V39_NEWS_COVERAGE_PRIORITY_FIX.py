...
# =========================================================
# NEWS - FINNHUB
# V39: coverage-priority fix after 31 Aug-12 Sep comparison.
# =========================================================
...
def _material_query_for_symbol(symbol):
    ...
    if " " in company:
        company = f'"{company}"'
    # V39: broaden focused discovery around events that proved easy to miss in V38.
    # Keep this event-driven: no analyst ratings/opinion terms are added here.
    material = (
        'recall OR recalled OR recalling OR investigation OR probe OR regulator OR antitrust OR '
        'lawsuit OR settlement OR fine OR subpoena OR acquisition OR merger OR guidance OR outlook OR forecast OR '
        '"raises guidance" OR "cuts guidance" OR "raises outlook" OR "cuts outlook" OR '
        '"revenue target" OR "revenue targets" OR "profit warning" OR earnings OR results OR '
        '"CEO resigns" OR "CEO steps down" OR "FDA approval" OR "FDA rejects" OR '
        '"price increase" OR "price increases" OR "price hike" OR "price hikes" OR discount OR incentive OR '
        '"cost increase" OR "cost increases" OR "raises prices" OR "higher prices" OR '
        'strike OR layoffs OR "job cuts" OR contract OR order OR partnership OR collaboration OR '
        'investment OR invests OR "to invest" OR buyback OR repurchase OR offering OR '
        '"launch event" OR "launch date" OR launch OR permit OR approval OR production OR deliveries OR sales OR '
        'tariff OR "export restriction" OR licensing OR "data center" OR cloud OR "AI infrastructure"'
    )
...
def score_news_item(symbol, item):
    ...
    if headline_subject_positions:
        ...
        if first_subject_symbol != str(symbol).upper().strip():
            # V39: a real two-company transaction is relevant to BOTH named companies.
            # Keep rejecting catalyst/comparison articles such as "X's Nvidia catalyst",
            # but allow explicit partnerships, contracts, investments and acquisitions.
            multi_company_event = any(x in headline_norm for x in [
                "partners with", "partnership", "collaboration", "collaborates with",
                "deal with", "agreement with", "contract with", "awarded by",
                "invests in", "investment in", "to invest in", "acquires",
                "acquisition of", "merger with", "supplies", "supplier to",
            ])
            if not (multi_company_event and _headline_mentions_symbol(symbol, headline)):
                print(
    ...
    # V39: analyst ratings/price-target notes are not company events. They created
    # 8/10-9/10 noise while contracts, regulation and guidance could be missed.
    analyst_note_markers = [
        "price target", "rating upgrade", "rating downgrade", "upgrades to buy",
        "downgrades to", "initiates coverage", "initiated coverage",
        "analyst says", "analyst sees", "analyst expects", "wall street sees",
        "buy candidate", "hold to", "accumulate", "outperform rating",
        "underperform rating", "overweight rating", "equal weight rating",
    ]
    if any(x in headline_norm for x in analyst_note_markers):
        print(f"NEWS DROP ANALYST NOTE {symbol}: {headline[:140]}", flush=True)
        return None
    ...
    # V39: event-family recovery. These are concrete corporate developments that
    # may use wording not present in the original keyword table.
    financial_change_markers = [
        "raises revenue forecast", "raised revenue forecast", "raises revenue target",
        "raised revenue target", "raises revenue targets", "raised revenue targets",
        "raises full-year", "raised full-year", "raises annual", "raised annual",
        "boosts forecast", "boosted forecast", "lifts forecast", "lifted forecast",
        "cuts forecast", "cut forecast", "lowers forecast", "lowered forecast",
        "revenue rises", "revenue jumps", "revenue falls", "revenue drops",
    ]
    if any(x in combined for x in financial_change_markers):
        importance, reason = max(importance, 9), "نتائج أو توقعات مالية"

    capital_markers = [
        "share repurchase", "stock repurchase", "repurchase program", "buyback program",
        "treasury operations", "capital allocation", "convertible bond", "convertible bonds",
    ]
    if any(x in combined for x in capital_markers):
        importance, reason = max(importance, 8), "تخصيص رأس مال أو تمويل مؤثر"

    legal_reg_markers = [
        "antitrust", "department of justice", "doj investigation", "ftc",
        "regulatory scrutiny", "regulatory probe", "licensing practices",
        "state department approved", "state department approves", "government approves",
        "senate bill", "new law", "signed into law", "regulators scrutinize",
    ]
    if any(x in combined for x in legal_reg_markers):
        importance, reason = max(importance, 8), "تنظيم أو تحقيق جوهري"

    strategic_markers = [
        "strategic investment", "strategic partnership", "expanded partnership",
        "expands partnership", "cloud agreement", "infrastructure agreement",
        "ai infrastructure", "data center expansion", "data centre expansion",
        "production order", "government contract", "military sale",
    ]
    if any(x in combined for x in strategic_markers):
        importance, reason = max(importance, 8), "صفقة أو شراكة استراتيجية"

    operating_markers = [
        "cash incentive", "cash incentives", "direct discount", "direct discounts",
        "sales rose", "sales rise", "sales fell", "sales fall",
        "deliveries rose", "deliveries rise", "deliveries fell", "deliveries fall",
        "production rose", "production rise", "production fell", "production fall",
    ]
    if any(x in combined for x in operating_markers):
        importance, reason = max(importance, 8), "تغير تشغيلي أو مبيعات مؤثر"

    # If it has no material-news trigger, do not show it.
    if importance < 7:
        return None

    # Direction classification. Material neutral/unclear events are kept as watch items
    # instead of being discarded completely; this was a major source of V38 misses.
    ...
    if pos >= 1 and pos > neg:
        ...
        # V39: a confirmed 8+/10 corporate event can matter even before its price
        # direction is obvious (regulation, new law, large strategic investment, etc.).
        # Keep it visible as a watch item rather than silently losing the news.
        if importance >= 8:
            sentiment, sentiment_ar = "WATCH", "🟡 مراقبة"
        else:
...
_NEWS_SOURCE_PRIORITY = {
    "reuters": 100,
    "sec": 100, "securities and exchange commission": 100,
    "department of justice": 100, "justice.gov": 100,
    "federal trade commission": 100, "ftc": 100,
    "u.s. army": 100, "us army": 100,
    "nhtsa": 100, "bls": 100,
...
def format_combined_news():
    ...
    # V39: manual "أهم الأخبار" shows the rolling freshness window instead of
    # Saudi-calendar "today" only. A fresh 11:55 PM story must not disappear at midnight.
