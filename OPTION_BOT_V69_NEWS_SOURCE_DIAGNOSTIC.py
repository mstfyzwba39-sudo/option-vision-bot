# OPTION_BOT V69 - News source diagnostics for Stock News API / Finnhub / Google RSS
import os
import time
import math
import asyncio
...
def get_stocknews_primary_news(force=False):
    ...
    if (not force and cached is not None and
        ...
        print(f"V69 PROVIDER STOCKNEWS CACHE HIT: {len(cached)} normalized items", flush=True)
        return list(cached)
    ...
    try:
        ...
        print(f"V69 PROVIDER STOCKNEWS HTTP {r.status_code}; payload_type={type(payload).__name__}; raw_items={len(raw_items) if isinstance(raw_items, list) else 'N/A'}", flush=True)
        ...
        print(f"V69 PROVIDER STOCKNEWS SYMBOLS: {sorted({sym for item in items for sym in (item.get('stocknews_tickers') or [])})}", flush=True)
        return list(items)
    except Exception as exc:
...
def _v69_log_final_stock_news(items, stage="FINAL"):
    counts = {}
    for x in items or []:
        provider = _v69_provider_name(x)
        counts[provider] = counts.get(provider, 0) + 1
        print(
            f"V69 {stage} PROVIDER={provider} SYMBOL={x.get('symbol','?')} "
            f"SOURCE={x.get('source','?')} IMPORTANCE={x.get('importance','?')} "
            f"HEADLINE={str(x.get('headline') or '')[:180]}",
            flush=True,
        )
    print(f"V69 {stage} PROVIDER COUNTS: {counts}", flush=True)
...
def format_combined_news():
    ...
    _v69_log_final_stock_news(stock_items, "MANUAL_FINAL")
...
async def post_init(application):
    ...
    if STOCK_NEWS_API_TOKEN or FINNHUB_TOKEN:
        ...
        if STOCK_NEWS_API_TOKEN:
            ...
            print("V69 NEWS SOURCE DIAGNOSTIC ENABLED — provider tags only in Render logs")
        else:
