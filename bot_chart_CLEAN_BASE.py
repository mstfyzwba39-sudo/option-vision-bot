...
def make_chart(symbol, resolution):
    ...
    # CLEAN BASE: candles first. No forecast engine, no demand zones, no extra models.
    ...
    ax.text(future_end + 0.45, setup["trigger"], f"CONFIRM  {setup['trigger']:.2f}",
            color=level_color, fontsize=10.2, va="center", fontweight="bold")
    ...
    ax.text(future_end + 0.45, setup["invalidation"], f"INVALID  {setup['invalidation']:.2f}",
            color=invalid, fontsize=9.8, va="center", fontweight="bold")
    ...
    for idx, target in enumerate(targets, 1):
        ...
        ax.text(future_end + 0.45, target, f"T{idx}  {target:.2f}",
                color=gold, fontsize=9.5, va="center", fontweight="bold")
    ...
    ax.text(x_last + 0.5, last_price, f"{last_price:.2f}",
            color=charcoal, fontsize=9.3, va="center",
    ...
    ax.set_title(
        ...
        fontsize=12.5, fontweight="bold", color=charcoal, pad=12, loc="left"
    )

    ax.yaxis.tick_right()
    ax.yaxis.set_label_position("right")
    ax.tick_params(axis="y", colors="#69645E", labelsize=8.8, length=0, pad=4)
