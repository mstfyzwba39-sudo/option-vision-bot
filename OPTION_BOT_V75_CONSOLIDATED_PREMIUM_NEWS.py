# OPTION_BOT V75 - Consolidated Stock News API quality + trial coverage
import os
import time
import math
import asyncio
...
def scan_top10(symbols=None):
    ...
    for symbol in target_symbols:
        try:
            # V58 — TOP OPPORTUNITIES uses the underlying stock direction as a HARD gate.
            # Bullish stock -> CALL candidates only. Bearish stock -> PUT candidates only.
            # Neutral/unclear direction -> no TOP opportunity until direction becomes clear.
            # This change is intentionally limited to TOP OPPORTUNITIES; Best Contracts
            # can still evaluate both CALL and PUT independently.
            trend = get_stock_trend(symbol)
            ...
            # Only contracts aligned with the current stock direction may qualify
            # for TOP OPPORTUNITIES. Opposite-side contracts are excluded even if
            # their liquidity/activity score is stronger.
            matching = [
                contract
                for contract in contracts
                if (
                    str(contract.get("side", "")).upper() == trend_bias
            ...
            # Pick the strongest contract INSIDE the confirmed stock direction only.
            matching.sort(
                key=lambda x: (
                    -x["internal_score"],
                    -x["uoa_score"],
            ...
            # Opportunity-specific decision: contract/liquidity quality qualifies the
            # candidate, but 1H live confirmation is the real gate between 🟡 and 🟢.
            # A strong contract alone must never be shown as a green entry.
            # Reaching this point already guarantees direction alignment in V58.
            aligned = True
            ...
            decision = {
                ...
                "reason": "العقد متوافق مع اتجاه السهم الحالي",
                "rank": decision_rank,
            }

            # Invalid setups are not "best opportunities" at all.
            # Exclude them before sorting/formatting so the manual list never
            # pads itself with a red contract just to reach a larger count.
            if decision_state == "RED":
                continue
...
async def post_init(application):
    ...
    if STOCK_NEWS_API_TOKEN or FINNHUB_TOKEN:
        ...
        if STOCK_NEWS_API_TOKEN:
            ...
            print("V75 CONSOLIDATED NEWS QUALITY ENABLED — provider tags only in Render logs")
        else:
